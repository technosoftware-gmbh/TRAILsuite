...
export function planCalendarImport(
  ...
  // The same meeting under a new identity.
  //
  // A UID is not as stable as it looks. Splitting a recurring series -- which
  // is what "change this and all following events" does -- ends the old series
  // at the edit and starts a new one, and Google names the new one after its
  // own start: `..._R20260915T113000`. Nothing about next Tuesday's meeting
  // moved, and yet it answers to a different UID in this export than it did in
  // the last one, so a comparison on uid and day alone reports it cancelled
  // while the very same run lists it as already present.
  //
  // So the key is asked as well: same day, same clock, same text is the same
  // meeting, whatever the file calls it, and that is the identity a person
  // reading the note can see (§D). It can only take a line off this list,
  // never put one on it, and what it costs is the case where a meeting really
  // was cancelled and another with the same wording put in the same slot --
  // which is not a thing to protect against at the price of calling a standing
  // meeting cancelled every time somebody edits the series.
  const stillHereByKey = new Set(proposals.map((one) => one.key));
  ...
  for (const line of wrote.values()) {
    ...
    if (stillHereByKey.has(line.key)) continue;

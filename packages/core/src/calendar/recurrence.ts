 ...
 *
 * **An occurrence is placed where the override puts it, and the window is
 * applied there.** The two directions are one rule and both used to be wrong:
 * an instance moved into the window from a rule day outside it was missing
 * from the week it now falls in, and an instance moved out of the window was
 * imported anyway, into a day outside the range the person chose.
 */
export function expandEvents(
  ...
  // Rule days lying **outside** the window whose override lands inside it.
  //
  // The rule is only ever walked across the window, and the override for a day
  // is looked up once that day has been walked. So an instance dragged out of
  // last Thursday and dropped onto this Monday is absent from the very week it
  // now sits in: the day the rule made is not in the window and is never asked
  // about. Recorded here so the series can ask about it by name.
  //
  // Only days outside the window, because one inside is walked anyway and
  // would arrive twice.
  const movedIn = new Map<string, string[]>();
  for (const event of events) {
    if (event.recurrenceId === null) continue;
    const ruleDay = dayOfIcsValue(event.recurrenceId);
    overrides.set(`${event.uid} ${ruleDay}`, event);
    const moved = event.start?.date ?? '';
    if (moved < from || moved > to) continue;
    if (ruleDay >= from && ruleDay <= to) continue;
    movedIn.set(event.uid, [...(movedIn.get(event.uid) ?? []), ruleDay]);
  }
  ...
  for (const event of events) {
    ...
    let days: string[];
    if (rule === null) {
      // A single event has no rule to place an override against, so nothing
      // can have been moved into the window on its behalf.
      days = start.date >= from && start.date <= to ? [start.date] : [];
    } else {
      const walked = ruleDays(rule, start.date, from, to);
      let gaveUp = walked.truncated;
      days = walked.days;
      // Each day carried in from outside the window is put to the rule on its
      // own, rather than the window being padded by some number of days wide
      // enough to look safe. An override may move an instance by a month, so a
      // pad is either a guess that is sometimes too small or a walk across
      // ground nobody asked about. Asking is exact, and costs one walk per
      // override that actually lands inside the range.
      //
      // The rule has to agree the day is one of its own: a RECURRENCE-ID left
      // behind by a rule that has since changed names a day the series no
      // longer falls on, and honouring it would put back an instance that is
      // not in the calendar.
      for (const ruleDay of movedIn.get(event.uid) ?? []) {
        const carried = ruleDays(rule, start.date, ruleDay, ruleDay);
        gaveUp = gaveUp || carried.truncated;
        if (carried.days.length > 0) days.push(ruleDay);
      }
      if (gaveUp) truncated.push({ uid: event.uid, summary: event.summary });
    }
    ...
    for (const day of days) {
      ...
      if (override) {
        ...
        // Where the override puts it, not where the rule would have. A rule
        // day inside the window whose instance was pushed into next month is
        // not this window's business, and importing it writes a line into a
        // day nobody asked about. No matching guard below: a day that reached
        // this loop from outside the window arrived with an override, so the
        // plain branch cannot be out of range.
        if (moved.date < from || moved.date > to) continue;

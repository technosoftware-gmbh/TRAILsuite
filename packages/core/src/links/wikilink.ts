...
/** The same again, keeping the alias, which is the half a reader actually sees. */
...
/**
 * A line of body text with every wikilink replaced by what a reader sees.
 *
 * `[[Stavanger]]` becomes `Stavanger` and `[[Stavanger|die Stadt]]` becomes
 * `die Stadt`. For anything that prints a note's own words somewhere Obsidian
 * is not rendering them -- a brochure, a prospect, a sheet somebody folds into
 * a pocket -- where a link's target is a fact about the vault and no use at
 * all to the person holding the paper.
 *
 * The alias wins when there is one, which is the whole point of writing one.
 * Without an alias the target is used with its subpath dropped, because
 * `Stavanger#Sehenswuerdigkeiten` names a heading in a file the reader does
 * not have; a link that is nothing **but** a subpath keeps it, since dropping
 * it would leave the sentence with a hole in it.
 *
 * An embed comes out as its target's name, the same as a link. A sheet cannot
 * embed anything, and the alternative is deleting a line of somebody's summary
 * without saying so.
 *
 * The reverse of nothing: this is display, and there is no reading it back.
 */
export function displayWikilinks(text: string): string {
  return text.replace(WIKILINK_DISPLAY, (whole, rawTarget: string, alias?: string) => {
    if (alias !== undefined && alias.trim() !== '') return alias.trim();

    const target = (rawTarget ?? '').trim();
    if (target === '') return whole;

    const withoutHeading = withoutSubpath(target);
    return withoutHeading === '' ? target : withoutHeading;
  });
}

...
/**
 * A list of lines, from a list or from a single value.
 *
 * For a property whose entries are **sentences**: a trip's highlights, an
 * excursion's. Not `readStringList` for `readPathList`'s reason and a worse
 * case of it -- "Starten Sie Ihre Reise im pulsierenden Oslo - die schoene,
 * historische Hauptstadt" would arrive as three highlights, each a fragment,
 * and the note would look like somebody had typed it that way.
 *
 * A single string is a list of one, because `highlights: Nostalgische
 * Zugreise` means one highlight rather than none.
 */
export function readTextLines(value: unknown): string[] {
  return wholeEntries(value);
}

/**
 * The two above, which are one rule: an entry is whole, and order is the
 * note's.
 *
 * They stay two exported names rather than one, because what a caller is
 * saying when it picks one is which mistake it is guarding against, and a
 * single name would answer neither question.
 */
function wholeEntries(value: unknown): string[] {
  const values = Array.isArray(value) ? (value as unknown[]) : [value];

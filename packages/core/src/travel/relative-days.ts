 ...
 *
 * **Here rather than in APERtrail because it is a statement about the file.**
 * `day: 3` on a stop means the third day of the trip whoever reads it, and
 * there is a second reader now: NODAtrail seeds a day note's entries from an
 * itinerary, and a relative stop it could not date would be a stop silently
 * skipped. APERtrail re-exports it, so none of its ten call sites moved.
 */

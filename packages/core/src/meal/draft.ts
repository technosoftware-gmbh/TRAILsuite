...
export interface MealDraft {
  ...
  description: string;
  /** How long to reheat for, in minutes. */
  reheatTime: number | null;
  /**
   * How hot, verbatim as the packet states it: `95 °C`, `800 W`.
   *
   * Free text rather than a number, because a steamer and an oven state
   * degrees while a microwave states watts, and those are not the same
   * quantity. One numeric field with a vault-wide unit could express only one
   * of them, and a second field for the unit would be a second thing a hand
   * edit can contradict. It is also what fills a supplier's `{temp}` token,
   * which has always substituted a string.
   */
  reheatTemp: string | null;
  /**
   * Which appliance, as an id from the consumer's appliance list.
   *
   * The id, never the display label: a label is translated and a note is not,
   * so storing the label would orphan every note in a vault that switches
   * language. Same separation as the appliance headings a note may carry.
   */

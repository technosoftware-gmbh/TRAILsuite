...
function row(overrides: Partial<BankStatementRow> = {}): BankStatementRow {
  return {
    ...
    text: 'MUSTERVERSICHERUNG AG',
    rawText: ' Belastung e-banking / Ref.-Nr. 1563958107 MUSTERVERSICHERUNG AG ',
    amount: -30.35,
    currency: 'CHF',
    balance: -1735.11,
...
describe('matchBillForRow', () => {
  it('matches on the amount and the vendor', () => {
    ...
    expect(match?.bill.title).toBe('20260801_Musterversicherung_1000000001');
    expect(match?.alsoFits).toEqual([]);
  });

  it('tells two bills from one vendor on one day apart by the amount', () => {
    // Both of these are real: 27.07.2026 carried MUSTERVERSICHERUNG 750.95 and MUSTERVERSICHERUNG 2.10.

...
/**
 * Links in body text, for anything printing a note somewhere Obsidian is not.
 *
 * The prospect printed `[[Stavanger]]` with its brackets, which is what
 * passing a callout's raw text to a `<p>` does. The target is a fact about the
 * vault and no use at all to whoever is holding the paper.
 */
describe('displayWikilinks', () => {
  it('prints a link as the note it names', () => {
    expect(displayWikilinks('Ab [[Stavanger]] weiter.')).toBe('Ab Stavanger weiter.');
  });

  it('prefers the alias, which is the whole point of writing one', () => {
    expect(displayWikilinks('Ab [[Stavanger|die Stadt]].')).toBe('Ab die Stadt.');
  });

  it('resolves every link on the line', () => {
    expect(displayWikilinks('[[A]] und [[B|beides]]')).toBe('A und beides');
  });

  /** A heading names a file the reader does not have. */
  it('drops a subpath nobody can follow', () => {
    expect(displayWikilinks('[[Stavanger#Sehenswertes]]')).toBe('Stavanger');
  });

  /** Dropping it would leave the sentence with a hole in it. */
  it('keeps a link that is nothing but a subpath', () => {
    expect(displayWikilinks('Siehe [[#Praktisches]].')).toBe('Siehe #Praktisches.');
  });

  /** A sheet cannot embed anything, and deleting the line silently is worse. */
  it('prints an embed as its name', () => {
    expect(displayWikilinks('![[deck.jpg]]')).toBe('deck.jpg');
  });

  it('leaves a line with no links exactly as it was', () => {
    expect(displayWikilinks('Nichts zu tun [hier].')).toBe('Nichts zu tun [hier].');
  });
});

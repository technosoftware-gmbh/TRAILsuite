...
describe('filling a form from the company it names', () => {
  const MUSTERVERSICHERUNG = { account: 4031, category: 'health' };

  it('fills an empty form', () => {
    expect(inheritFromCompany(NOTHING, MUSTERVERSICHERUNG)).toEqual(MUSTERVERSICHERUNG);
  });

  it('overwrites what was there, because somebody just chose this company', () => {
    expect(inheritFromCompany({ account: 4036, category: 'other' }, MUSTERVERSICHERUNG)).toEqual(
      MUSTERVERSICHERUNG
    );
  });
  ...
  it('keeps everything else on the form untouched', () => {
    ...
    expect(inheritFromCompany(form, MUSTERVERSICHERUNG)).toEqual({
      ...MUSTERVERSICHERUNG,
      amount: 92.15,
      reference: 'X-1',
    });
  ...
  it('does not mutate what it was given', () => {
    ...
    inheritFromCompany(form, MUSTERVERSICHERUNG);

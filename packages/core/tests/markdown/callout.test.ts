...
import { calloutLines, calloutText, findCallout, findCallouts } from '../../src/markdown/callout';
...
/**
 * Every callout of a kind, which is a different question from the first.
 *
 * It exists so a form can say something about a note that carries two
 * summaries. Everything that reads one takes the first, so the second is text
 * nothing shows, nothing prints and nothing edits -- and the note looks fine.
 */
describe('findCallouts', () => {
  it('finds one where there is one', () => {
    expect(findCallouts(NOTE, 'SUMMARY')).toHaveLength(1);
  });

  it('finds both where somebody pasted twice', () => {
    const note = ['> [!SUMMARY]+', '> First.', '', 'Dazwischen.', '', '> [!summary]', '> Second.'];

    expect(findCallouts(note, 'SUMMARY').map((found) => found.lines.join(''))).toEqual([
      'First.',
      'Second.',
    ]);
  });
  ...
  it('finds none where there are none', () => {
    expect(findCallouts(NOTE, 'INFO')).toEqual([]);
  });

  /** Scanning resumes past the quoted lines, so a nested callout is the outer one's text. */
  it('does not find a callout nested inside another', () => {
    const note = ['> [!SUMMARY]+', '> Above.', '> > [!SUMMARY] Inner', '> > Nested.'];

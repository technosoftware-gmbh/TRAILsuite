...
describe('planCalendarImport', () => {
  ...
  describe('what is no longer in the export', () => {
    ...
    it('does not flag a meeting whose UID the export reissued', () => {
      // "Change this and all following events" splits the series: the old one
      // is ended at the edit and a new one starts, named after its own start.
      // Nothing about the meeting moved. Compared on uid and day alone it is
      // announced as cancelled by the same run that lists it as already
      // present two rows further up.
      const still = line('2026-09-14', '09:00', 'Meeting mit Care Management');
      const out = plan(file(CARE), {
        existing: [still],
        history: [
          {
            ...
            lines: [
              {
                uid: 'care@example.ch_R20260101T090000',
                day: '2026-09-14',
                key: meetingKey('2026-09-14', '09:00', 'Meeting mit Care Management'),
              },
            ],
...
describe("the zone the file states, read into the vault's", () => {
  ...
  it('derives a prior key off the converted clock, not the file digits', () => {
    ...
    // and `missing` compares uid and day before it ever consults a key. The
    // one observable difference is the key itself.
    const ics = file(
      event(
        'UID:z@example.ch',
        'DTSTART:20260914T060000Z',

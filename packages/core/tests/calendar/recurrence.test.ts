...
describe('expandEvents', () => {
  ...
  it('finds an instance an override moved into the window from a rule day outside it', () => {
    // The window is a working week and the rule places this one on the Monday
    // before it. Walking the rule across the week alone never reaches 7
    // September, so the override that drags the meeting onto the Tuesday is
    // never asked about and the week it now sits in comes back without it.
    const moved = event(
      'UID:standup@example.ch',
      'RECURRENCE-ID;TZID=Europe/Zurich:20260907T090000',
      'DTSTART;TZID=Europe/Zurich:20260915T110000',
      'SUMMARY:Standup, verschoben'
    );
    const out = expandEvents(
      parseIcs(file(STANDUP, moved)),
    ...
    expect(out.map((one) => `${one.date} ${one.time ?? ''}`)).toEqual([
      '2026-09-14 09:00',
      '2026-09-15 11:00',
    ]);
    expect(out[1]).toMatchObject({ summary: 'Standup, verschoben', overridden: true });
  });

  it('leaves out an instance an override moved beyond the window', () => {
    // The other direction of the same rule, and the one that reaches a vault:
    // the Monday is inside the week, the meeting that replaced it is a
    // fortnight away, and importing it writes a line into a day nobody asked
    // about.
    const pushed = event(
      'UID:standup@example.ch',
      'RECURRENCE-ID;TZID=Europe/Zurich:20260914T090000',
      'DTSTART;TZID=Europe/Zurich:20260928T090000',
      'SUMMARY:Standup, verschoben'
    );
    expect(
      expandEvents(parseIcs(file(STANDUP, pushed)), '2026-09-14', '2026-09-20').occurrences
    ).toEqual([]);
  });

  it('refuses a RECURRENCE-ID naming a day the rule does not fall on', () => {
    // A stale override, left behind by a rule that has since changed. The day
    // it names is a Wednesday and the series runs on Mondays, so there is no
    // instance for it to replace, and honouring it would put back a meeting
    // that is not in the calendar.
    const stale = event(
      'UID:standup@example.ch',
      'RECURRENCE-ID;TZID=Europe/Zurich:20260909T090000',
      'DTSTART;TZID=Europe/Zurich:20260915T110000',
      'SUMMARY:Standup, verschoben'
    );
    expect(
      expandEvents(parseIcs(file(STANDUP, stale)), '2026-09-14', '2026-09-20').occurrences.map(
        (one) => one.date
      )
    ).toEqual(['2026-09-14']);
  });

  it('does not carry in an instance whose rule day the series excludes', () => {
    // EXDATE removes the instance, and an override replaces an instance. One
    // that is not there has nothing to replace.
    const excluded = event(
      'UID:ferien@example.ch',
      'DTSTART;TZID=Europe/Zurich:20240108T090000',
      'DTEND;TZID=Europe/Zurich:20240108T091500',
      'RRULE:FREQ=WEEKLY;BYDAY=MO',
      'EXDATE;TZID=Europe/Zurich:20260907T090000',
      'SUMMARY:Standup'
    );
    const moved = event(
      'UID:ferien@example.ch',
    ...
    expect(
      expandEvents(parseIcs(file(excluded, moved)), '2026-09-14', '2026-09-20').occurrences.map(
        (one) => one.date
      )
    ).toEqual(['2026-09-14']);
  });

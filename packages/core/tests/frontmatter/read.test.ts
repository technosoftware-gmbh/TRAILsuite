...
/**
 * The list reader for properties whose entries are sentences.
 *
 * Its whole reason for existing is the comma. `readStringList` splits on one,
 * which is right for `diet: vegetarian, gluten-free` and ruinous for a
 * highlight: a line naming "Oslo - die schoene, historische Hauptstadt" would
 * arrive as two highlights, each a fragment, and the note would read as though
 * somebody had typed it that way.
 */
describe('readTextLines', () => {
  it('keeps an entry whole, commas and all', () => {
    expect(readTextLines(['Oslo, die Hauptstadt'])).toEqual(['Oslo, die Hauptstadt']);
  });

  it('reads a single value as a list of one', () => {
    expect(readTextLines('Nostalgische Zugreise')).toEqual(['Nostalgische Zugreise']);
  });

  it('trims, and drops what says nothing', () => {
    expect(readTextLines([' a ', '', '   ', 7, null])).toEqual(['a']);
    expect(readTextLines(undefined)).toEqual([]);
  });

  it("keeps the note's own order, which is the order somebody meant them read in", () => {
    expect(readTextLines(['dritter', 'erster', 'zweiter'])).toEqual([
      'dritter',
      'erster',
      'zweiter',
    ]);
  ...
  /** The difference, stated as a test rather than only in a docstring. */
  it('differs from readStringList on exactly one input', () => {
    expect(readStringList('a, b')).toEqual(['a', 'b']);
    expect(readTextLines('a, b')).toEqual(['a, b']);
  });
});

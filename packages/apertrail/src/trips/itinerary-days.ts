...
import { ArrivingLeg, legDayNumbers } from './leg-days';
...
export function itineraryDays(
  ...
  const insert = (number: number, title: string | null, note: string | null): void => {
    if (covered.has(number)) return;

    const at = groups.findIndex((group) => group.number !== null && group.number > number);
    const group: ItineraryDayGroup = {
      date: dateOfDay(departure, number),
      number,
      title,
      note,
      stops: [],
  ...
  for (const day of [...days].sort((a, b) => a.day - b.day)) insert(day.day, day.title, day.note);
  // A leg leaves on a day the trip itself may not have: the outbound flight
  // the evening before day one, the return the day after the last. Those days
  // are drawn rather than skipped, because a leg named on no day at all is
  // how the flight came to be missing from the day-by-day -- and a day that
  // exists is a day somebody can name and write a paragraph on, like any
  // other. See leg-days.ts for which days a leg asks for.
  for (const number of legDayNumbers(legs, departure)) insert(number, null, null);

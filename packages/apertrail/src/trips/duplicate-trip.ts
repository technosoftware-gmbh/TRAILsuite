...
export function duplicateTripInput(input: TripInput, rehome: TripRehome): TripInput {
  return {
    ...
    reviewStatus: null,
    rating: null,
    // Cleared with the dates and the status, and for the same reason: what
    // the copy follows on from is a fact about when it happens, and the copy
    // happens nowhere yet. A duplicate that kept the link would put a second
    // extension under a parent nobody edited.
    extendsTitle: null,

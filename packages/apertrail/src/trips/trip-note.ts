...
export interface TripStopInput extends TripLineChoiceInput {
  ...
  /** Which excursion this stop is, or null. A title; the writer turns it into a wikilink. */
...
export interface TripPropertyNames {
  ...
  /**
   * The trip this one follows on from.
   *
   * Written on the CHILD. The parent's list of extensions is derived at read
   * time instead, so adding three days in Kopenhagen in December does not
   * reopen and rewrite a voyage note finished in September, and so nothing
  ...
  /**
   * Which excursion the stop is, as a link.
   *
   * Separate from `place`, which is where it happens: Stavanger is not the
   * Viking tour. What it costs stays on this line, because the same tour is
   * priced differently on every sailing that offers it.
   */
...
export interface TripFrontmatterInput {
  ...
  personTitles: string[];
  /** The trip this one follows on from, as a title. Null for a trip that stands alone, which is most of them. */
...
export function buildTripFrontmatter(input: TripFrontmatterInput): Record<string, unknown> {
  ...
  // Written as a plain wikilink, with no guard here on purpose: this builder
  // does not know the note's own title (a trip's title is its filename), and
  // the self-link a guard would catch is one somebody types into frontmatter
  // by hand, which no builder ever sees. It is refused where it is read
  // instead -- see readTravelBoard's extensions pass.
  const extendsTitle = cleanString(input.extendsTitle);
  if (extendsTitle) yaml[p.extendsProperty] = toWikilink(extendsTitle);
  ...
  const stops = input.stops
    .filter(
      (stop) =>
        ...
        // A stop that names only its excursion is a real line: an outing
        // whose meeting point is the ship says what it is without saying
        // where, and dropping it would lose the one fact it carries.
        cleanString(stop.excursionTitle) !== null ||
    ...
    .map((stop) => {
      ...
      const excursion = cleanString(stop.excursionTitle);
      if (excursion) entry[p.stopExcursionField] = toWikilink(excursion);
...
export interface ParsedTripStop extends ParsedTripLineChoice {
  ...
  rating: number | null;
  motifName: string | null;
  /** Which excursion this stop is, as written. Null for the ordinary stop, which is most of them. */
  excursionTitle: string | null;
  cost: number | null;
  currency: string | null;
  costUnit: CostUnit;
...
export interface ParsedTripRecord {
  ...
  rating: number | null;
  personTitles: string[];
  /** The trip this one follows on from, as written. Null for a trip that stands alone. */
  extendsTitle: string | null;
  days: ParsedTripDay[];

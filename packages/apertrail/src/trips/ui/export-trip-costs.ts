...
import { estimateLines, optionalItems, optionalTotal } from '../costs/estimates';
...
export function buildCostSheet(
  ...
  // The rows behind that figure. The cell above says what all of them together
  // would add, which is a ceiling: two excursions offered on one day are a
  // choice between them, so nobody spends the sum. They stay out of the
  // bookings table below, which is what the totals and the settlement read.
  ...
  return {
    ...
    optional: extras.length === 0 ? null : { label: t('costs.optional'), lines: extras },
    totals: totalRows,
    balances:
...
export async function exportTripCostSheet(
  ...
  // The same folder the trip document lands in, because both are renderings of
  // the same note and the rule is about the note rather than about the sheet.
  const name = sanitizeTitle(`${trip.title} ${t('costs.sheetFileSuffix')}`);
  await writeSheet(app, exportPath(settings, trip.file.path, name), buildCostSheetHtml(sheet), {
    written: 'costs.sheetWritten',
    failed: 'costs.sheetFailed',
  });
}

...
import { cabinsForVehicle, SuggestableVehicle, vehiclesForCarrier } from '../leg-suggestions';
 ...
 */
/**
 * A text field that suggests what the vault has and accepts anything.
 *
 * The datalist keeps a vocabulary discoverable without enforcing it, which is
 * the shape the travel-type field already uses. Every field it serves here is
 * one where a note may simply not exist -- an airline, a coach, a cabin on a
 * ship somebody has not written down -- so a dropdown would be a control that
 * refuses the ordinary case.
 *
 * The list id is derived from the field so two of them on one form do not share
 * one list, which is a bug that looks like the wrong suggestions appearing.
 */
function renderSuggestingField(
  ...
  const setting = new Setting(containerEl).setName(labels.name);
  if (labels.desc) setting.setDesc(labels.desc);
  setting.addText((text) => {
    if (labels.placeholder) text.setPlaceholder(labels.placeholder);
    text.setValue(value ?? '').onChange((raw) => {
      onChange(raw.trim() === '' ? null : raw);
    });
    attachSuggestions(text.inputEl, suggestions, listId);
  });
}

/** Hangs a datalist off one input. Drawn nowhere when there is nothing to suggest, so an empty list is not an empty dropdown. */
function attachSuggestions(
  ...
  const datalist = inputEl.parentElement?.createEl('datalist', { attr: { id: listId } });
  for (const suggestion of suggestions) {
    datalist?.createEl('option', { attr: { value: suggestion } });
  }
  inputEl.setAttr('list', listId);
}
...
function renderPriceFields(
  ...
  kind: 'stop' | 'night' | 'leg',
  rerender: () => void,
  /** The names this line's variants could carry: a ship's cabins, on a leg that names one. Empty everywhere else. */
  variantNames: readonly string[] = []
): void {
  if (value.variants.length === 0) {
    renderCostField(containerEl, costLabel, value, settings, kind);
  }
  renderVariantFields(containerEl, costLabel, value, settings, kind, rerender, variantNames);
  ...
  value.variants.forEach((variant, index) => {
    ...
    new Setting(containerEl)
      ...
      .addText((text) => {
        ...
        // The ship's own cabins, where the leg names a ship. Spelling this the
        // same way on both sides is what makes the description appear under
        // the price; typing it twice is what stops it.
        attachSuggestions(text.inputEl, variantNames, 'apt-variant-name-suggestions');
      })
      .addToggle((toggle) =>
        toggle
...
export class StopEditorModal extends ItemEditorModal<TripStopInput> {
  ...
  /**
   * The excursion notes the vault has, read once when the dialog opens
   * rather than on each redraw: the board is rebuilt from disk on every
   * read, and this form redraws whenever a day number or a price changes.
   * The same reason the leg dialog caches its ships.
   */
  private excursionTitles: string[] | null = null;

  private knownExcursions(): string[] {
    this.excursionTitles ??= readTravelBoard(this.app, this.settings).excursions.map(
      (x) => x.title
    );
    return this.excursionTitles;
  }
  ...
  protected validate(): string | null {
    ...
    const saysSomething =
      this.value.placeTitle.trim() !== '' ||
      (this.value.note ?? '').trim() !== '' ||
      // Or an excursion: an outing that meets on board names no place and
      // may carry no prose either, and it is still a real line.
      (this.value.excursionTitle ?? '').trim() !== '';
  ...
  protected renderFields(containerEl: HTMLElement): void {
    ...
    // What the stop IS, as against where it is. A suggesting field rather
    // than a picker, for the reason the leg's ship has one: a tour somebody
    // typed the name of, with no note behind it yet, is still the tour they
    // are taking, and the reader matches raw names for exactly that case.
    renderSuggestingField(
      containerEl,
      {
        name: t('modals.stopEditor.excursionField'),
        desc: t('modals.stopEditor.excursionDesc'),
        placeholder: t('modals.stopEditor.excursionPlaceholder'),
      },
      this.knownExcursions(),
      this.value.excursionTitle,
      (value) => {
        this.value.excursionTitle = value;
      },
      'apt-stop-excursion-suggestions'
    );
    ...
    // words of it.
    new Setting(containerEl)
      .setName(t('modals.tripEditor.stopNote'))
      .setClass('apt-form-multiline')
      .addTextArea((text) =>
...
export class LegEditorModal extends ItemEditorModal<TripLegInput> {
  /**
   * What the vault knows, read once when the dialog opens rather than on each
   * redraw: the board is rebuilt from disk on every read, and this form
   * redraws whenever a day number or a price changes.
   */
  private suggestions: { companies: string[]; vehicles: SuggestableVehicle[] } | null = null;

  private known(): { companies: string[]; vehicles: SuggestableVehicle[] } {
    if (!this.suggestions) {
      const board = readTravelBoard(this.app, this.settings);
      this.suggestions = {
        companies: readCrmBoard(this.app, this.settings).companies.map((company) => company.title),
  ...
  protected renderFields(containerEl: HTMLElement): void {
    ...
    renderSuggestingField(
      containerEl,
      {
        name: t('modals.tripEditor.legVehicle'),
        desc: t('modals.tripEditor.legVehicleDesc'),
        placeholder: t('modals.tripEditor.legVehiclePlaceholder'),
      },
      // Narrowed by the carrier above, and never to nothing: a carrier that
      // matches no operator leaves every ship on offer.
      vehiclesForCarrier(this.known().vehicles, this.value.carrier).map((vehicle) => vehicle.title),
      this.value.vehicleTitle,
      (value) => {
        this.value.vehicleTitle = value;
        // Redrawn, because the cabins offered below are this ship's.
        this.render();
      },
      'apt-leg-vehicle-suggestions'
    );
    ...
    renderSuggestingField(
      containerEl,
      {
        name: t('modals.tripEditor.legCarrier'),
        desc: t('modals.tripEditor.legCarrierDesc'),
        placeholder: t('modals.tripEditor.legCarrierPlaceholder'),
      },
      this.known().companies,
      this.value.carrier,
      (value) => {
        this.value.carrier = value;
        // Redrawn, because the ships offered below are this operator's.
        this.render();
      },
    ...
    renderPriceFields(
      ...
      'leg',
      () => this.render(),
      cabinsForVehicle(this.known().vehicles, this.value.vehicleTitle)
    );
    renderTravellersField(containerEl, this.value, this.participants);
  }
}

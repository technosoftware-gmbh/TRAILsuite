...
class TripCostsRenderer extends MarkdownRenderChild {
  ...
  private render(): void {
    ...
    this.renderSummary(
      el,
      totals,
      optionalTotal(trip, estimateLabels(), currency),
      optionalItems(trip, estimateLabels()).map((item) => ({
        label: item.label,
  ...
  private renderSummary(
    ...
    /** Those extras one by one, already formatted, in the note's own order. */
    ...
    // The rows behind the optional tile. That tile is a ceiling: two excursions
    // offered on the same day are a choice between them, so the sum is a figure
    // nobody spends, and the rows are what the choice is actually made from.
    if (extras.length > 0) {
      const list = container.createDiv({ cls: 'apt-trip-costs-extras' });
      for (const extra of extras) {
        const row = list.createDiv({ cls: 'apt-trip-costs-extra' });
        row.createSpan({ cls: 'apt-trip-costs-extra-label', text: extra.label });
        row.createSpan({ cls: 'apt-trip-costs-extra-amount', text: extra.amount });
      }
    }

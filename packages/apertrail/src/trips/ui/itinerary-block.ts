...
import { stopInsertIndex } from '../stop-insert';
import {
  dayAnchor,
  ScheduleConflict,
  scheduleConflicts,
...
class ItineraryRenderer extends MarkdownRenderChild {
  ...
  private renderStops(trip: TravelTrip): void {
    // The legs as well as the stops: a day the outbound flight leaves on is a
    // day of the itinerary even when nothing else is booked on it. See
    // itinerary-days.ts.
    const groups = itineraryDays(trip.stops, trip.departure, trip.days, trip.transport);
    ...
    for (const group of groups) {
      ...
      // What ends here and what leaves here. A leg lives in the transport band
      // and is edited there; these are the lines saying the voyage finishes
      // and the flight goes today, which the band cannot say because it has no
  ...
  /**
   * One line under a day saying a leg leaves or lands on it.
   *
   * A marker, not a row: no price, no booking, no actions. The leg's own row
   * in the transport band is where all of that is, and two places to edit one
   * leg is how a note ends up disagreeing with itself.
   */
  private renderLegLine(leg: TravelTripLeg, text: string): void {
    const line = this.el.createDiv({ cls: 'apt-itinerary-arrival' });
    setIcon(line.createSpan({ cls: 'apt-itinerary-arrival-icon' }), travelModeIcon(leg.mode));
  ...
  /**
   * What the stop IS, under where it is.
   *
   * The leg's own line is the model: the name reads whether or not the vault
   * has a note for it, and the link is drawn only where there is something to
   * open. A tour somebody typed the name of is still the tour they are
   * taking, and a row that hid it until a note existed would hide the fact.
   *
   * The duration rides along when the note carries one, because on a day page
   * "eight hours" is what decides whether the rest of the day is free.
   */
  private renderStopExcursion(body: HTMLElement, stop: TravelTripStop): void {
    if (!stop.excursionTitle) return;
    const line = body.createDiv({ cls: 'apt-itinerary-note apt-itinerary-excursion' });
    setIcon(line.createSpan({ cls: 'apt-chip-icon' }), 'compass');
    if (stop.excursion) {
      const link = line.createEl('a', {
        cls: 'apt-itinerary-link',
        text: stop.excursionTitle,
      });
      link.addEventListener('click', (event) => {
        event.preventDefault();
        this.deps.openFile(stop.excursion?.file.path ?? '');
      });
    } else {
      line.createSpan({ text: stop.excursionTitle });
    }
  ...
  private renderStopRow(trip: TravelTrip, stop: TravelTripStop, index: number): void {
    ...
    this.renderStopExcursion(body, stop);
  ...
  private renderTransport(trip: TravelTrip): void {
    ...
    trip.transport.forEach((leg, index) => {
      ...
      setIcon(row.createSpan({ cls: 'apt-itinerary-icon' }), travelModeIcon(leg.mode));
      ...
      if (leg.vehicle) {
        ...
        // The deck plan beside her name, because the moment you want it is
        // the moment you are choosing between the cabins listed just below.
        // Only when the vault actually has the file: a link to a path that
        // resolves to nothing would open an empty tab and say nothing.
        const plan = leg.vehicle.deckPlan ? resolveReference(this.app, leg.vehicle.deckPlan) : null;
        if (plan && plan.kind !== 'missing') {
          open.createSpan({ text: ' · ' });
          // A plan on the web is a real anchor and nothing intercepts it:
          // Obsidian opens an external href in the system browser, which is
          // where a PDF nobody has downloaded belongs. A plan in the vault is
          // a tab, like the ship's note above it.
          const planLink = open.createEl('a', {
            cls: plan.kind === 'url' ? 'apt-itinerary-link external-link' : 'apt-itinerary-link',
            text: t('itinerary.deckPlan'),
            href: plan.kind === 'url' ? plan.url : undefined,
          });
          if (plan.kind === 'file') {
            const path = plan.file.path;
            planLink.addEventListener('click', (event) => {
              event.preventDefault();
              this.deps.openFile(path);
            });
          }
        }
      }
      ...
      renderRowActions(row, [
        ...
        }),
        {
          ...
          label: t('modals.tripEditor.moveUp'),
          disabled: index === 0,
          onClick: () => this.moveLine(trip, (input) => input.transport, index, -1),
        },
        {
          icon: 'arrow-down',
          label: t('modals.tripEditor.moveDown'),
          disabled: index === trip.transport.length - 1,
          onClick: () => this.moveLine(trip, (input) => input.transport, index, 1),
        },
        {
          icon: 'trash-2',
    ...
    trip.nights.forEach((night, index) => {
      ...
      renderRowActions(row, [
        ...
        }),
        {
          ...
          label: t('modals.tripEditor.moveUp'),
          disabled: index === 0,
          onClick: () => this.moveLine(trip, (input) => input.nights, index, -1),
        },
        {
          icon: 'arrow-down',
          label: t('modals.tripEditor.moveDown'),
          disabled: index === trip.nights.length - 1,
          onClick: () => this.moveLine(trip, (input) => input.nights, index, 1),
        },
        {
          icon: 'trash-2',
  ...
  private addStop(trip: TravelTrip, group: ItineraryDayGroup | null): void {
    ...
    new StopEditorModal(
      ...
      (value) => {
        this.save(trip, (input) => {
          // Inserted at its day rather than appended: the list is the
          // itinerary's order, so a stop added under day 3 and written to the
          // end opens a second day-3 group below the last day of the trip.
          input.stops.splice(stopInsertIndex(input.stops, value, trip.departure), 0, value);
        });
      },
      trip.personTitles,
      trip.departure
  ...
  /**
   * The same swap, for the bands that are a plain list.
   *
   * Legs and stays are written in the order the note lists them and read in
   * that order everywhere -- the band, the printed document, the cost sheet
   * -- so a leg added for the first day arriving at the bottom of the band is
   * a row in the wrong place with no way to move it. The list is picked out
   * of a fresh `TripInput` by the caller, which is the arrangement the
   * variants and the optional chip already use.
   */
  private moveLine<T>(
    trip: TravelTrip,
    pick: (input: TripInput) => T[],
    index: number,
    delta: number
  ): void {
    this.save(trip, (input) => {
      moveInList(pick(input), index, delta);
    });
  }

...
import { ParsedTripLeg } from './trip-note';
...
/**
 * What to call a leg where the route is the whole of the line.
 *
 * Where it goes, then who runs it for a leg that names no ends, then a word
 * for the leg that says neither. One chain in one place: the itinerary block
 * and the trip document each wrote it out, and they have to agree, since the
 * printed sheet is what somebody checks the screen against.
 */
export function legRouteText(leg: ParsedTripLeg): string {
  return legRoute(leg, t('itinerary.legJoiner')) ?? leg.carrier ?? t('itinerary.unnamedLeg');
}

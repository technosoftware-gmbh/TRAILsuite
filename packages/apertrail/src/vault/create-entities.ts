...
import { buildExcursionFrontmatter } from '../places/excursion-note';
import { excursionProperties, vehicleProperties } from './read-entities';
...
import { relatedTripsBody } from './related-trips-body';
...
export async function createVehicleNote(
  ...
  settings: APERtrailSettings,
  title: string,
  draft: { mode?: string | null; operatorTitle?: string | null; description?: string | null } = {},
  now: Date = new Date()
): Promise<TFile> {
...
/**
 * A guided tour, a safari, a bus to the Nordkap.
 *
 * Creation collects what somebody knows when they first write the tour down
 * -- who runs it, how long it takes, and where -- and nothing else. There is
 * deliberately no price field anywhere on this note: the same excursion costs
 * one thing on a Christmas sailing and another in May, so the figure belongs
 * to the stop that books it.
 *
 * It carries the related-trips block from the moment it exists, so the note
 * answers "which trips did this" as soon as one does.
 */
export async function createExcursionNote(
  app: App,
  settings: APERtrailSettings,
  title: string,
  draft: {
  ...
  const rest = buildExcursionFrontmatter(
    {
      description: draft.description ?? null,
      operatorTitle: draft.operatorTitle ?? null,
    ...
    excursionProperties(settings)
  );

  const content =
    renderFrontmatterBlock(
      frontmatterObject(propertyName(settings), 'excursion', createdEntry(settings, now), rest)
    ) + relatedTripsBody();
  return createNote(app, settings.excursionsFolder, title, content);
}

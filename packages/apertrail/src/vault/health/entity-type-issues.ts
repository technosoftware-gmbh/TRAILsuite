/**
 * Scans APERtrail's fourteen entity folders for notes whose `type:`
 * frontmatter is missing or disagrees with the folder they sit in.
 *
 * Every one of the fourteen folders maps to exactly one entity type, which is
 * the unambiguous case this kind of check handles well: there is always a
 * confident suggestion, never a guess.
 *
 * The twelve travel folders expect a fixed literal; the two CRM folders
...
function locationConfigs(settings: APERtrailSettings): LocationConfig[] {
  const configs: LocationConfig[] = [
    ...
    { location: 'excursions', folder: settings.excursionsFolder, expectedType: 'excursion' },

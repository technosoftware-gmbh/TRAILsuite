 ...
 *
 * It also carries every warning the other files in this folder produce --
 * photo spots, bookings, references to files the vault does not have, and
 * variants naming a cabin their ship does not list. They sit in lists of
 * their own with an Open button and no Set button, because none of them has
 * a suggestion the plugin may apply: the fix is a decision, not a value this
 * code could compute.
 */
...
function describeMissingFile(issue: MissingFileIssue): string {
  return issue.detail
    ? t('health.missingFileCheck.inList', {
...
export class EntityTypeCheckModal extends Modal {
  ...
  private render(): void {
    ...
    // One call each, and the order is the order they are about: the two
    // note-type checks first, then the two that are about a mistake any note
    // can make, with the file-reference one last because it is the only
    // section that reports on every type at once.
    this.renderWarningList(contentEl, this.spotIssues, 'photoSpotCheck', describeWarning);
    this.renderWarningList(contentEl, this.bookingIssues, 'bookingCheck', describeBookingWarning);
    this.renderWarningList(
      contentEl,
      this.variantCabinIssues,
      'variantCabinCheck',
  ...
  /**
   * A heading, a sentence and a row per warning, hidden entirely when there
   * are none.
   *
   * Two sections had grown this shape separately and identically; a third and
   * a fourth arriving is what made it a shape rather than a coincidence. An
   * empty section with a reassuring sentence would be the fifth thing this
   * modal says about a vault that is already fine, and the paragraph at the
   * top says it once already.
   *
   * `section` is the locale key rather than two rendered strings, because the
   * heading takes the count and passing it in at every call site was four
   * copies of `{ count: issues.length }` waiting to disagree with the list
   * beneath them.
   *
   * Every warning here carries the note it is about, so the row finds the file
   * itself. None of them carries a fix: what a path was meant to say, and
   * which of two notes is the duplicate, are answers only the person who wrote
   * them has -- which is why this has an Open button and no Set button.
   */
  private renderWarningList<T extends { file: TFile }>(
    containerEl: HTMLElement,
    issues: T[],
    section: string,
    describe: (issue: T) => string
  ): void {
    if (issues.length === 0) return;

    containerEl.createEl('h3', {
      text: t(`health.${section}.heading`, { count: issues.length }),
    });
    containerEl.createEl('p', {
      text: t(`health.${section}.explain`),
      cls: 'setting-item-description',
    });

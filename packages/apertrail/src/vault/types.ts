...
import { ParsedExcursion } from '../places/excursion-note';
...
export interface TravelPlace {
  ...
  /** What it shows on a cover: a line, a picture, the rest of them, and its highlights. See vault/read-cover.ts. */
  description: string | null;
  image: string | null;
  gallery: ParsedTripPicture[];
  highlights: string[];
...
/**
 * A guided tour, a boat safari, a bus to the Nordkap: the thing a stop is
 * sold as an outing rather than as a visit.
 *
 * Not a place -- it is run rather than gone to -- and not a trip, for the
 * reason recorded in places/excursion-note.ts. It carries no price of its
 * own: the same tour costs one thing on a Christmas sailing and another in
 * May, so the figure belongs to the stop that books it. Its country and city
...
export interface TravelTrip {
  ...
  personTitles: string[];
  /**
   * The trip this one follows on from, as written, and the same resolved.
   *
   * The link lives on the CHILD: three days in Kopenhagen are planned long
   * after the voyage note was finished, and a parent that had to list them
   * would be reopened every time one was added. A title the vault has no
   * trip for is kept and resolves to null, the way every unresolved link
   * here does.
   */
   ...
   *
   * **One level, never a chain.** A extends B extends C puts C on B and B on
   * A, and no reader walks further, which is what makes a cycle terminate by
...
export interface TravelTripStop extends ParsedTripLineChoice {
   ...
   * target; a name the vault has no note for stands as typed, the same way a
   * leg's vehicle does.
   *
   * Beside `place` rather than instead of it: on the Nordkap's day 3 the
   * place says Stavanger and the excursion says which tour, and both are
   * true at once.
   */

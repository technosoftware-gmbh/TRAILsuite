 ...
 *
 * `accommodationType`, `accommodationStatus` and `fnbType` were once read at
 * fixed names, as the three documented exceptions to this plugin's "every
 * property name is a setting" rule. They have settings now, because the place
 * editor writes them, and a name this plugin writes is a name a vault has to be
 * able to choose. See docs/design/data-model.md.
 */
  ...
  return [
    {
      ...
      properties: {
        ...
        [settings.accommodationTypeProperty]: 'Hotel',
        [settings.accommodationStatusProperty]: 'Booked',
      },
      body: relatedTripsBody(),
      ensureBlock: TRAVEL_RELATED_TRIPS_BLOCK_LANG,
    },
...
function trips(settings: APERtrailSettings): SampleNote[] {
  const rovos: TripInput = {
    ...
    stops: [
      ...
      // The optional line that started all of this: an outing sold on the
      // day, priced per head, and described in a note of its own rather than
      // retyped into every trip that takes it. `chosen` is what puts it in
      // the plan; the excursion note holds what it is.
      //
      // It names no place, deliberately. A tour that meets at the hotel and
      // spends the day going round a peninsula is not a visit to one note,
      // and this is the line that exercises the writer's own rule: a stop
      // naming only its excursion is a real stop. It also keeps Cape Town
      // out of the visit derivation, which the trip is careful not to touch.
      {
        placeTitle: '',
        day: null,
        from: '2026-02-12T08:00',
        to: '2026-02-12T16:30',
        note: null,
        rating: null,
        motifName: null,
        excursionTitle: 'Cape Peninsula by road',
        cost: 1450,
        currency: 'ZAR',
        costUnit: 'person',
        persons: [],
  ...
  /**
   * The trip that follows another one.
   *
   * Two days in the Winelands after the train reaches Cape Town: its own
   * dates, its own nights, its own budget, and a single `extends:` naming
   * what it comes after. The link is on this note and not on the voyage,
   * which is the whole shape of the relation -- the Rovos note was finished
   * before anybody thought of the Winelands, and adding this one did not
   * reopen it. Its extensions are derived when the board is read.
   */
  const winelands: TripInput = {
    subtitle: 'Two nights in Stellenbosch, after the train',
    image: null,
    highlights: [],
    gallery: [],
    countryTitle: 'South Africa',
    cityTitles: ['Cape Town'],
    departure: '2026-02-15T10:00',
    return: '2026-02-17T16:00',
    travelType: null,
    travelStatus: 'Over',
    reviewStatus: null,
    rating: null,
    personTitles: ['Stefan'],
    extendsTitle: 'Rovos Rail 2026',
    days: [],
    stops: [],
    nights: [],
    transport: [],
  ...
  return [
    ...
    {
      folder: newTripFolder(settings, 'Winelands 2026'),
      title: 'Winelands 2026',
      typeValue: tripType,
      properties: tripProperties(settings, winelands),
      body: tripBody(
        'Hired a car in Cape Town and drove out to Stellenbosch for two nights, once the ' +
          'train had done its part.'
      ),
    },
    {
 ...
 *
 * One note, and it is the outing the sample's own trip takes at the far end
 * of the line, so the `excursion:` on that stop resolves to something a
 * reader can open. It carries no price, which is the whole rule: the same
 * tour costs one thing in February and another in December, so the figure is
 * on the stop that books it and the description is here.
 *
 * No `operator:` on this one, though it takes the same link a vehicle's
 * does. The only company in this vault runs the train, and pointing a road
 * tour at it to demonstrate a field would be a fact the sample invented.
 */
function excursions(settings: APERtrailSettings): SampleNote[] {
  return [
    {
      folder: settings.excursionsFolder,
      title: 'Cape Peninsula by road',
      typeValue: 'excursion',
      properties: {
        [settings.descriptionProperty]:
          'Chapman’s Peak, the Cape of Good Hope and the penguins at Boulders, in one day.',
        [settings.excursionDurationProperty]: 'About 8 hours',
        [settings.countryProperty]: link('South Africa'),
        [settings.cityProperty]: link('Cape Town'),
        [settings.websiteProperty]: 'https://peninsula-tours.example/by-road',
      },
      body: relatedTripsBody(),
      ensureBlock: TRAVEL_RELATED_TRIPS_BLOCK_LANG,
    },

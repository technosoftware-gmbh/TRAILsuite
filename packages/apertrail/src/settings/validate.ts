...
/**
 * The exports subfolder, carrying a renamed setting across.
 *
 * `tripExportsSubfolder` covered trips only and defaulted to `Exports`;
 * `exportsSubfolder` covers every sheet and defaults to `_exports`, which is
 * the name the vault already uses for `_resources` and `_documents`.
 *
 * **A value somebody typed themselves is kept; the old default is not.** A
 * vault that never touched the setting was not choosing `Exports`, it was
 * taking whatever came, and the new answer is the one it should now take. A
 * vault that did type its own folder name meant it, and silently renaming
 * their folder would be the plugin overruling a decision.
 */
function exportsSubfolder(r: Record<string, unknown>, fallback: string): string {
...
/** What `tripExportsSubfolder` used to default to. Kept only so the migration above can tell a chosen value from an inherited one. */
const RETIRED_TRIP_EXPORTS_DEFAULT = 'Exports';

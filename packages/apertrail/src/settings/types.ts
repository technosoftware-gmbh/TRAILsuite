...
export interface APERtrailSettings {
  ...
  // `tags:` on a City or place, and the two subtypes' own fields.
  //
  // The last three were this plugin's documented exceptions to the
  // every-name-is-a-setting rule, read at fixed names because each belongs to
  // one subtype rather than to the shared place shape, and `tags` simply never
  // had a setting. Reading a literal is one thing; the place editor writes
  // these, and a name this plugin writes is a name a vault has to be able to
  // choose. The defaults are the literals they replace, so no note changes.
  tagsProperty: string;
  ...
  /**
   * The trip this one follows on from, as a link.
   *
   * On the CHILD, never on the parent: the three days in Kopenhagen are
   * planned months after the voyage note was finished, and the parent's list
   * of extensions is derived at read time rather than written into a note
   * somebody would have to reopen. Nothing derived is written back.
   */
  tripExtendsProperty: string;
  ...
  /** Which excursion a stop is, as a link. Separate from the place it happens at: Stavanger is not the Viking tour. */
  ...
  /** A deck plan or a train layout: one document per vehicle, linked rather than shown. */
  vehicleDeckPlanProperty: string;
  ...
  cabinImageField: string;

  // ══════════════════════════════════════════════════════════════════════
  // The excursion: the outing a stop is sold as, as a note of its own. It
  // carries no price -- the same tour is priced differently on every sailing
  // that offers it, so the figure stays on the trip's own line. See
  // docs/design/excursions.md.
  // ══════════════════════════════════════════════════════════════════════
  excursionsFolder: string;

...
export interface VehiclePropertyNames {
  /**
   * The one-line description, sharing the name a Person and a Company note
   * already use rather than inventing a second word for the same thing. The
   * long prose stays in the note body, where a reader writes it.
   */
  ...
  /**
   * A deck plan, or a train's layout: one document, as a vault path or a
   * wikilink. Not `image`, because nothing can inline it -- it is a PDF, and
   * every surface that shows it shows a link to it.
   */
  deckPlanProperty: string;
  ...
  /** A picture of the cabin, as a vault path or a wikilink -- the same shape a gallery entry's image takes. */
...
export interface ParsedVehicleCabin {
  ...
  /** A picture of it, exactly as the note wrote it: a vault path, a wikilink or a URL. Null for a cabin nobody has photographed. */
  image: string | null;
}
...
export interface ParsedVehicle {
  /** One or two lines: what she is, in the words a brochure would use. The long version is the note body. */
  description: string | null;
  ...
  tonnage: string | null;
  website: string | null;
  /** Exactly as the note wrote it: a vault path or a wikilink. Resolving it is the caller's job, the same as for a picture. */
  deckPlan: string | null;
  image: string | null;
...
export function buildVehicleFrontmatter(
  ...
  write(p.deckPlanProperty, input.deckPlan);
  ...
  const cabins = input.cabins
    ...
    .map((cabin) => {
      ...
      const image = cabin.image?.trim();
      if (description) entry[p.cabinDescriptionField] = description;
      if (image) entry[p.cabinImageField] = image;
      return entry;
    });

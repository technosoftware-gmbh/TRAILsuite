...
export function buildFieldSheetHtml(sheet: FieldSheet): string {
  ...
  // The polar note belongs to the sun block and prints with it or not at all,
  // which is why this asks about the rows rather than handing section() a list
  // that a polar note alone could keep alive.
  const sun =
    sheet.sun.length === 0
      ? ''
      : section(sheet.dateLine, [
          `<div class="sun">${sheet.sun
            .map(
              (row) =>
                `<div><div class="k">${esc(row.label)}</div><div class="v">${esc(
                  row.value
                )}</div></div>`
            )
            .join('')}</div>`,
  ...
  const motifs = section(
    sheet.labels.motifs,
    sheet.motifs.map((motif) => motifBlock(motif, sheet.labels))
  );

  const loose = section(
    sheet.labels.samples,
    sheet.looseSamples.length === 0 ? [] : [samplesBlock(sheet.looseSamples)]
  );

...
export class SampleEditorModal extends PhotoSpotItemModal<PhotoSpotSampleInput> {
  constructor(
    ...
    /**
     * The photo spot this sample belongs to, which is where a picture chosen
     * from the machine is filed. Optional so a caller that does not know can
     * still open the dialog, and the field then offers the picker alone --
     * the rule `imageField` states and every caller of it follows.
     */
  ...
  protected renderFields(container: HTMLElement): void {
    // A sample IS a picture, so this is the one field in the dialog that has
    // to be reachable without leaving it: a frame you just exported is in the
    // vault, or on the disk, and typing its path from memory is the step
    // nobody manages.
    imageField(container, {
      app: this.app,
      label: t('modals.sampleEditor.image'),
      description: t('modals.sampleEditor.imageDesc'),
      get: () => this.value.image,
      set: (value) => {
        this.value.image = value;
      },
      refresh: () => this.render(),
      notePath: this.notePath ? () => this.notePath ?? '' : undefined,
    });

...
export function renderPlaceStatsRow(
  ...
  // The one tile with a single number. Every other one here is a progress
  // fraction, and there is no denominator to put under a ship: a vehicle is
  // not visited, not captured, and there is no set of all ships to have seen
  // some of. "3 / 3" would read as an achievement rather than a count.
  renderNumberStatCard(row, {
    value: String(stats.vehiclesTotalCount),
    label: t('dashboard.stats.vehicles'),
    onClick: () => actions.showType('vehicle'),
  });
}

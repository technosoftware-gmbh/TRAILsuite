 ...
 *
 * It sits at the top because it is the only part of the page that is about
 * the plugin rather than about a vault, and because "what changed in the
 * version I just updated to" is asked exactly once per update, at the moment
...
export function renderSectionHeader(containerEl: HTMLElement, host: Plugin, app: App): void {
  const card = sectionCard(containerEl);

  new Setting(card)
    .setName(t('settings.header.whatsNew.name', { version: host.manifest.version }))
    .setDesc(t('settings.header.whatsNew.desc'))
    .addButton((button) =>
      button
        .setButtonText(t('settings.header.whatsNew.button'))
        .onClick(() => new WhatsNewModal(app, host.manifest.version).open())
    );

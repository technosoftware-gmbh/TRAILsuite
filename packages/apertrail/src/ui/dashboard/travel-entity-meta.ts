...
import { travelModeIcon, travelModeLabel } from '../../shared/travel-mode';
...
/**
 * What she is, who runs her, and how old she is.
 *
 * The three facts that tell two ships apart in a grid, and the only three
 * a vehicle carries that every vehicle has some chance of carrying. The
 * numbers a brochure leads with -- capacity, length, tonnage -- are not
 * here: they answer a question you ask once you have picked the ship,
 * and the brochure is where they belong.
 *
 * Built and refurbished share one chip rather than taking two. They are one
 * fact about her age said in two parts, and a card row of four chips reads
 * as a table.
 */
export function vehicleMetaItems(vehicle: TravelVehicle): EntityCardMetaItem[] {
  const items: EntityCardMetaItem[] = [];
  const mode = travelModeLabel(vehicle.mode);
  if (mode) items.push({ icon: travelModeIcon(vehicle.mode), text: mode });
  if (vehicle.operatorTitle) items.push({ icon: 'building-2', text: vehicle.operatorTitle });
  // A ship refurbished on a build year nobody wrote down still says
  // something: "Umbau 2023" alone is better than nothing at all.
  const refurbished = vehicle.refurbished
    ? t('dashboard.vehicleRefurbished', { year: vehicle.refurbished })
    : null;
  const age = [vehicle.built, refurbished].filter((x): x is string => !!x).join(' · ');
  if (age) items.push({ icon: 'calendar', text: age });
  // Presence, not a link: a card meta item is a label and an icon, and the
  // grid opens the note on a click. What this answers is which of the ships
  // you have a plan for, which is a question you ask of the whole grid rather
  // than of one card.
  if (vehicle.deckPlan) items.push({ icon: 'file-text', text: t('itinerary.deckPlan') });
  return items;
}

/**
 * Where it is, who runs it, and how long it takes.
 *
 * The three facts that tell two tours apart in a grid. Deliberately no price:
 * an excursion note carries none, because the same tour is priced differently
 * on every sailing that offers it, and a card that showed one would be
 * quoting a figure the note does not have.
 */
export function excursionMetaItems(excursion: TravelExcursion): EntityCardMetaItem[] {
  const items: EntityCardMetaItem[] = [];
  const where = excursion.city?.title ?? excursion.country?.title ?? null;
  if (where) items.push({ icon: 'map-pin', text: where });
  if (excursion.operatorTitle) items.push({ icon: 'building-2', text: excursion.operatorTitle });
  if (excursion.duration) items.push({ icon: 'clock', text: excursion.duration });
  return items;
}

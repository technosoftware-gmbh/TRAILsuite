 ...
 * that happens to know who you went with. Vehicles sort by name for the same
 * reason and sit ahead of them, where their chip does: a ship is a thing you
 * travelled on, which is nearer the subject than a person you travelled with.
 */
export function defaultGalleryRanks(board: TravelBoard, crmBoard: CrmBoard): Map<string, number> {
  const ordered = [
    ...
    ...[...board.vehicles].sort(byName),
    // Excursions sort by name for the vehicles' reason and sit beside them:
    // neither carries a date, a rating or a visit of its own, and both are
    // things a trip names rather than places a trip goes.
    ...[...board.excursions].sort(byName),

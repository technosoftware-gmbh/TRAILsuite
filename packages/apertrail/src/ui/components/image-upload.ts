/**
 * Putting a file that is not in the vault yet into the vault.
 *
 * **It says "picture" less than it used to**, because the writing half was
 * always general: `getAvailablePathForAttachment` does not care what the bytes
 * are, and a deck plan needs filing beside its note exactly as a photograph
 * does. Only the CHOOSING is per kind, which is why the accept lists and the
 * choosers below are separate and the writer is one.
 *
...
/**
 * What a document chooser should offer.
 *
 * Deliberately wider than "a PDF": a deck plan is usually one, and a scan, a
 * photograph of a printed plan and an emailed brochure are all the same thing
 * to somebody filing it. The vault-side picker takes the same view -- anything
 * that is not a note -- so the two halves of the field agree about what a
 * document is.
 */
export const DOCUMENT_ACCEPT = '.pdf,.png,.jpg,.jpeg,.webp,.svg,.txt,.eml,.doc,.docx';
...
export function chooseImageFiles(): Promise<File[]> {
  return chooseFiles(IMAGE_ACCEPT, true);
}

/**
 * The same, for one document.
 *
 * One rather than many, because the fields that take a document take exactly
 * one: a vehicle has a deck plan, not a deck plan list. A chooser offering a
 * multiple selection to a field that keeps the first would be discarding
 * answers somebody gave.
 */
export function chooseDocumentFile(): Promise<File | null> {
  return chooseFiles(DOCUMENT_ACCEPT, false).then((files) => files[0] ?? null);
}

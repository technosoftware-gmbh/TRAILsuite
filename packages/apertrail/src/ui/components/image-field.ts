...
export interface ImageFieldOptions {
  ...
  label: string;
  /**
   * What the row says under its name, where the caller has something better to
   * say than the shape of the value. A cabin's picture is printed beside it on
   * the brochure and a sample frame is the sample; both are worth saying, and
   * neither is said by "a vault path, a wikilink, or a URL".
   */
  description?: string;
  get: () => string;
...
export function imageField(container: HTMLElement, options: ImageFieldOptions): void {
  const setting = new Setting(container)
    ...
    .setDesc(options.description ?? t('modals.common.imageFieldHint'))
    .addText((input) =>
  ...
  setting.addExtraButton((button) =>
    button
      ...
      .onClick(async () => {
        ...
        const { written, failed } = await uploadAttachments(options.app, [first], notePath());

...
APERtrail defines twelve fixed `type:` values (`src/vault/entity-types.ts`), all lowercase and unquoted:
...
| `vehicle` | Vehicle | The ship or named train a leg is taken on. Not a place: no coordinates, never a stop, and its cabins are a catalogue rather than prices, see [Vehicles](../design/vehicles.md) |
| `excursion` | Excursion | What a stop is sold as: a guided tour, a safari, a bus to the Nordkap. Not a place either -- it is run rather than gone to -- and it carries no price, see [Excursions](../design/excursions.md) |
...
- **`stops:`** the itinerary. Each stop is a place, an optional start and end time, an optional note, an optional 1 to 5 rating for that visit specifically, and optionally what it costs. A stop may say **which day of the trip** it is on (`day: 3`) instead of naming a date, and may name **no place at all** -- a time and a sentence is a whole entry. It may also name an **`excursion:`** beside the place: the tour it is, as against the town it happens in. See [an itinerary before its dates](#an-itinerary-before-its-dates) below and [Excursions](../design/excursions.md).
...
- **`extends:`** the trip this one follows on from, where it follows one -- three days in Kopenhagen after the ship docks. Written on the trip that comes second, never on the one it names, so adding an extension never reopens a finished note. What follows a trip is derived when the vault is read. See [Excursions](../design/excursions.md).
...
A stop added under a day is **inserted at that day**, after the last stop already on it, rather than appended to the end of `stops:`. That is not cosmetic: the list is the itinerary's order and nothing sorts it, so an appended day-3 stop opens a second day-3 heading below the last day of the trip. Placement reads whichever way the stop says when -- a day number on a relative trip, a date on a dated one -- because ordering on the number alone would leave every dated trip appending exactly as before. **Nothing already in the list moves.** A trip whose stops are deliberately out of day order stays as it is, only the new row is placed, and a stop that names no day at all is appended, because it belongs to whatever run it is written next to and only the person adding it knows which.
...
The same block in a **Vehicle** note answers which trips had a leg aboard it, and in an **Excursion** note which trips took that tour -- with their stops, because on a tour's own note the stops are the answer: which day of which trip, and what it cost that time.
...
trip's `_exports` folder: the summary, every booking and surviving estimate,
...
Every sheet this plugin exports lands in a subfolder of the folder its note is
in, named by `exportsSubfolder` (`_exports` by default) -- the same
relationship `_resources` and `_documents` have to the notes beside them. Notes
in one folder share one, so five flat trips in `Trips/` render into
`Trips/_exports/`. Blank writes every sheet beside its note instead.

**Every note with a card in the gallery can print a prospect** -- a ship, the
five place types, a city, a state, a country -- from the button on its
related-trips block or from *Export this note as a prospect*. See
[Prospects](../design/prospects.md).

**Every export opens what it wrote**, in a new tab, on a re-export as much as
on the first one. The note you exported from stays where it was.
...
`<Trip> document.html` into the same `_exports` folder. It is the
...
file in the note's exports folder: `<Spot> field sheet.html`. It carries the motifs with

...
**6 September 2026**, from a third real trip: a Nordkap voyage whose brochure
offers something at nearly every port. An excursion in Stavanger costing 119
francs a head printed as the bare word "Optional", and the figure was two pages
back in the cost table.

Which is backwards. Every other line on the page is describing something that
is happening, and what the trip costs is the cost section's job. **An optional
line is the one line the reader has to decide about while reading it**, and the
price is most of the decision. So the heading carries it: "Optional - CHF 119.00
per person", and "Optional, taken - CHF 119.00 per person" once somebody has.

Two things it deliberately does not do.

**It prints the unit with the amount**, rather than the amount alone, because
the unit changes the sum. 119 francs for two people is not what 119 francs per
person is, and this page is read by somebody deciding for a household as often
as for themselves. The four units the rest of the plugin uses are the four that
can appear here.

**A line priced through `variants:` says the bare word.** Its variants are
listed underneath it in full, with their own names and figures; repeating one of
them in the heading would be picking a favourite the note did not pick. The
first-variant assumption `lineFigure()` makes for a total is defensible in a
total -- the alternative is leaving the largest figure on the trip out of its
own budget -- and is not defensible in a heading that looks like a quoted price.

The label is `optionalLabel()` in the App-bound half, and it is **exported**,
which is unusual there. It is the join between a markup module and an
arithmetic one, and that join is where both of this file's recorded defects
lived. Its `trip` and `settings` parameters are required rather than optional
for the same reason: they exist only as the currency fallback, and an optional
parameter would have let a call site forget one and print a label with no price
-- silently, and only for the lines whose own note omits the currency. A
required parameter turns that into a typecheck instead of a test.
...
`exportsSubfolder` is the folder name, `_exports` by default, and it is a
subfolder of whatever folder the note is in -- the same relationship
`_resources` and `_documents` have to the notes beside them. A foldered trip
renders into `Trips/<trip>/_exports/`; a trip still flat in `Trips/` renders
into `Trips/_exports/`, shared with every other flat trip, the way they already
share one attachments folder. Blank writes sheets beside the note.

**The cost sheet moved with it.** It wrote beside the note and now goes through
the same `exportFolder()`, so a trip's renderings are one place rather than

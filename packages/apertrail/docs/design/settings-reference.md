...
the same names every other note with a card uses.

An excursion note carries **no price at all**, which is the vehicle's rule with
the catalogue half removed. The same tour costs one thing on a Christmas
sailing and another in May, so the figure lives on the stop that books it, in
its `cost` or its `variants`, and the description lives here. Correcting the
description corrects every trip that ever took it.

A vehicle also reads `descriptionProperty` -- the same one a Person and a
Company note use -- for the line the brochure prints under its name. The long
prose stays in the note body.

...
Places/                     everything a trip can point at
  ...
  Vehicles/ Excursions/     what you travel on, and what you are sold a day of
CRM/
  People/ Companies/
```
...
Every folder is a setting, and each module moves as a unit: repoint `placesFolder` and the ten folders under it follow -- eight place folders plus `Vehicles` and `Excursions`, neither of which is a place and both of which hang off the Places root, because every folder is derived from one of the three module roots and that is what keeps a module relocatable. An optional `rootFolder` sits above all three and defaults to empty, meaning the vault root. See [Settings reference](settings-reference.md#folders).
...
4. Reads vehicles and excursions, each in a single pass: a stop points at an excursion and a leg at a vehicle, and neither knows which trips took it.
5. Reads trips, because their stops point at Cities, places and excursions and their legs at vehicles, and all of those must be indexed first.
6. **Walks the trips a second time** to join `extends:` up. This is the step that ends the sentence which used to stand here -- *"nothing points back up at a Trip, so this is still one pass"* -- and it is the only reason a trip is not read in one. It mutates in place, exactly as step 2 does, so every reference points at the instances the rest of the board holds. Two guards make it safe: **a trip is never its own extension**, and **nothing walks a chain** -- `extensions` is one level deep, so a cycle of any length terminates by construction rather than by a visited-set somebody has to keep right. See [Excursions](excursions.md).
7. Runs visit derivation over the already-built objects **in place**, since every cross-reference from step 2 points at those exact instances and replacing them would leave the board pointing at stale copies.
...
Every derivation here follows the same rule, for the same reason: writing would mean editing notes as a side effect of editing a *different* note, and the derived value would go stale the moment its source changed. Explicit beats derived, every time.
...
Every note type carries that block, not only a Trip, and every editor offers it. What a sheet prints is the callout and nothing else in the note: text below it is the note owner's own working notes. The callout is read as the vault's markdown rather than as plain text, so a `- ` list prints as a list and `[[Stavanger|die Stadt]]` prints as "die Stadt". A save rewrites the quoted lines and leaves the opening line as the note spells it, which is what keeps a hand-written `> [!summary]` from being restyled by a form.
...
`excursion:` names the outing the line **is**, beside the `place:` that says
where it happens: Tromsø is not the dog-sled tour. It points at an
[excursion note](excursions.md), which holds the description, the picture and
the duration, while the price stays here, because the same tour is sold at a
different figure on every trip that offers it. A line may name an excursion and
no place at all -- an outing that meets on board is somewhere in general and
nowhere in particular -- and a name the vault has no note for is kept and shown,
the way a leg's vehicle is.
...
## What an editor writes, and what it refuses to

A place note is edited from its card or from its own related-trips block, and
so are a country, a state and a city. The dialog owns a narrower set of keys
than the note holds. `write-place.ts` clears what it owns and writes what
it means, so a field emptied in the dialog does not linger from before it, and
everything below is deliberately outside that boundary:

- **`visited` and `lastVisit`** are never written by the editor. A finished
  trip that stops somewhere already contributes them, and a dialog writing them
  back would give one answer two sources. See [Derived fields](#derived-fields).
- **`image`, `gallery`, `description` and `highlights`** are the note's cover,
  and they have their own schema (`write-cover.ts`) rather than belonging to
  any one type. `highlights` is the trip's own property, shared rather than
  duplicated: one name for one idea, so a vault renames it once. A type's own
  schema may read it, and none of them owns it.
  The editor collects them and hands them to the type's writer, which clears
  and writes both key sets in one pass. Where both schemas own a key -- a
  vehicle's `description` -- the type's own value is the one that survives.
- **The summary callout** is body text, not a property, so it is not on any
  writer's schema. The editor collects it beside the cover's description and
...
a setting at all. All four have one now, because the editor writes them and a
name this plugin writes is a name a vault has to be able to choose. The
defaults are the literals they replace.

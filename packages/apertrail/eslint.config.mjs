...
export default [
  ...
  // puts createEl on Node and on the global scope, not on Window.
  {
    files: [
      ...
      // The scratch canvas moved here when the ship brochure became the second
      // consumer of the downscaler. It has to move on this list with it: the
      // rule's autofix rewrote the line into `activeWindow.createEl('canvas')`,
      // which the paragraph above says does not exist, and `--fix` did it
      // silently on a file that had just passed everything.
      'src/shared/inline-picture.ts',

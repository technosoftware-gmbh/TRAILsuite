...
const P: VehiclePropertyNames = {
  ...
  descriptionProperty: DEFAULT_SETTINGS.descriptionProperty,
  highlightsProperty: DEFAULT_SETTINGS.tripHighlightsProperty,
  websiteProperty: DEFAULT_SETTINGS.websiteProperty,
  deckPlanProperty: DEFAULT_SETTINGS.vehicleDeckPlanProperty,
  ...
  cabinImageField: DEFAULT_SETTINGS.cabinImageField,
};
...
const INPUT = {
  ...
  cabins: [
    ...
    { name: 'Arktis Superior', description: null, image: null },
  ],
};
...
describe('what a vehicle note carries', () => {
  ...
  it('drops a cabin with no name, on the way out and on the way back', () => {
    const yaml = buildVehicleFrontmatter(
      { ...INPUT, cabins: [{ name: '   ', description: 'nothing to point at', image: null }] },
      P
    );
    expect(yaml).not.toHaveProperty('cabins');
   ...
   */
  /** The cabin's own picture goes with the cabin, and the brochure prints it beside the text. */
  it('keeps a picture on the cabin it belongs to', () => {
    const parsed = parseVehicle(buildVehicleFrontmatter(INPUT, P), P);

    expect(parsed.cabins[0]?.image).toBe('Places/Vehicles/_resources/polar.webp');
    expect(parsed.cabins[1]?.image).toBeNull();
  });

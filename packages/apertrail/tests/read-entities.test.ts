...
describe('a leg on a ship', () => {
  ...
  /**
   * The four names that had no setting until the place editor needed to write
   * them. A literal read is invisible until a vault renames one, and nothing
   * else in this suite points these settings anywhere but their defaults, so
   * without this the change from `fm.tags` to the setting could not go red.
   */
  it('honours renamed tag and subtype properties on a place', () => {
    const renamed = {
      ...settings,
      tagsProperty: 'schlagworte',
      accommodationTypeProperty: 'unterkunftsart',
      accommodationStatusProperty: 'buchungsstand',
      fnbTypeProperty: 'lokalart',
    };
    const { app } = makeFakeVault([
      {
        path: `${renamed.accommodationFolder}/Hotel Bristol.md`,
        frontmatter: {
          type: 'accommodation',
          schlagworte: ['ruhig'],
          unterkunftsart: 'Hotel',
          buchungsstand: 'Gebucht',
        },
      },
      {
        path: `${renamed.fnbFolder}/Cafe Central.md`,
        frontmatter: { type: 'fnb', lokalart: 'Cafe' },
      },
    ]);
    const places = readTravelBoard(app, renamed).places;
    const hotel = places.find((place) => place.kind === 'accommodation');
    const cafe = places.find((place) => place.kind === 'fnb');

    expect(hotel?.tags).toEqual(['ruhig']);
    expect(hotel?.accommodationType).toBe('Hotel');
    expect(hotel?.accommodationStatus).toBe('Gebucht');

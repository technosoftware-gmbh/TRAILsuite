...
describe('entity type check over Travel folders', () => {
  ...
  it('leaves correctly typed notes alone across all eleven travel folders', () => {
    expect(
      scan([
        ...
        // The two that are not places and sit under the Places root anyway.
        // Both were missing from this list for a while, which is how a folder
        // gets added to the check and never checked.
        { path: `${P}/Vehicles/J.md`, frontmatter: { type: 'vehicle' } },
        { path: `${P}/Excursions/K.md`, frontmatter: { type: 'excursion' } },
      ])
    ).toEqual([]);
  });

  it('suggests excursion for a mistyped note in the Excursions folder', () => {
    const issues = scan([
      { path: `${P}/Excursions/Cape Peninsula.md`, frontmatter: { type: 'location' } },
    ]);

    expect(issues).toHaveLength(1);
    expect(issues[0].location).toBe('excursions');
    expect(issues[0].suggestedType).toBe('excursion');
    expect(issues[0].reason).toBe('mismatch');
  });

  it('suggests vehicle for a mistyped note in the Vehicles folder', () => {
    const issues = scan([
      { path: `${P}/Vehicles/MS Trollfjord.md`, frontmatter: { type: 'location' } },
    ]);

    expect(issues[0].location).toBe('vehicles');
    expect(issues[0].suggestedType).toBe('vehicle');
  });

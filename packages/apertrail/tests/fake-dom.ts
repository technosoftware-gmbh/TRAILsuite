...
export interface FakeEl {
  tag: string;
  /** Set on an input a `Setting` component owns, because that is where a `<datalist>` is hung. */
...
export interface FakeControl {
  ...
  /** The input a text field was given, so a test can reach the datalist hung off its parent. */
  inputEl?: FakeEl;
  text?: string;
  icon?: string;
...
function component(entry: FakeControl): Record<string, unknown> {
  ...
  const self: Record<string, unknown> = {
    ...
    // A parent, because `attachSuggestions` hangs the `<datalist>` off it. A
    // component whose input had none would silently suggest nothing, and the
    // test asserting the suggestions would pass for the wrong reason.
    inputEl: input,
  };
  return self;
}
...
/**
 * The values a row's text field suggests, read out of the `<datalist>` the
 * editor hung off it.
 *
 * This is what a cascading picker actually produces, and the only way to tell
 * "offers this ship's cabins" from "offers nothing" without a browser.
 */
export function suggestionsFor(name: string): string[] {
  const control = rowsNamed(name)[0]?.controls.find((entry) => entry.kind === 'text');
  const datalist = control?.inputEl?.parentElement?.children.find(
    (child) => child.tag === 'datalist'
  );
  return (datalist?.children ?? []).map((option) => option.attrs.value ?? '');
}

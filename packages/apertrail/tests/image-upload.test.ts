...
import { uploadAttachments } from '../src/ui/components/image-upload';
...
interface Stub {
  app: Parameters<typeof uploadAttachments>[0];
  taken: Set<string>;
  order: string[];
}
...
describe('uploading a picture', () => {
  it('lets Obsidian decide where it goes, and asks about the right note', async () => {
    const stub = anApp();
    const result = await uploadAttachments(stub.app, [aFile('duene45.jpeg')], NOTE);
  ...
  it('does nothing at all when nothing was chosen', async () => {
    const stub = anApp();
    const result = await uploadAttachments(stub.app, [], NOTE);

...
import { buttonLabelled, control, resetSettings, rowsNamed, suggestionsFor } from './fake-dom';
...
/**
 * The cascade a leg fills in: who runs it, which ship, which cabin.
 *
 * Every field takes anything typed -- most airlines will never be a note --
 * and each narrows the next. Asserted on what the fields actually offer,
 * because the offering is the half a pure test cannot see.
 */
describe('the leg pickers', () => {
  const seeded = () =>
    makeFakeVault([
      {
        path: `${DEFAULT_SETTINGS.companiesFolder}/Hurtigruten.md`,
        frontmatter: { type: DEFAULT_SETTINGS.companyTypeValue },
      },
      {
        path: `${DEFAULT_SETTINGS.vehiclesFolder}/MS Trollfjord.md`,
        frontmatter: {
          type: 'vehicle',
      ...
      {
        path: `${DEFAULT_SETTINGS.vehiclesFolder}/Pride of Africa.md`,
        frontmatter: { type: 'vehicle', operator: '[[Rovos Rail Charters]]' },
      },
    ]);

  function openSeededLeg() {
    const modal = new LegEditorModal(
      seeded().app,
      DEFAULT_SETTINGS,
      aLegInput(),
  ...
  it('keeps the carrier and the ship as separate fields', () => {
    openLeg();

    expect(rowsNamed(t('modals.tripEditor.legCarrier'))).toHaveLength(1);
    expect(rowsNamed(t('modals.tripEditor.legVehicle'))).toHaveLength(1);
  });

  it('offers the companies the vault has, and every ship, to begin with', () => {
    openSeededLeg();

    expect(suggestionsFor(t('modals.tripEditor.legCarrier'))).toEqual(['Hurtigruten']);
    expect(suggestionsFor(t('modals.tripEditor.legVehicle'))).toEqual([
      'MS Trollfjord',
      'Pride of Africa',
    ]);
  });

  it('narrows the ships once an operator is named', () => {
    openSeededLeg();

    control(t('modals.tripEditor.legCarrier'), 'text')?.change?.('Hurtigruten' as never);

    expect(suggestionsFor(t('modals.tripEditor.legVehicle'))).toEqual(['MS Trollfjord']);
  });

  /** The point of the whole chain: name the ship and her cabins are what the price rows suggest. */
  it('offers that ship cabins under the prices', () => {
    openSeededLeg();

    control(t('modals.tripEditor.legVehicle'), 'text')?.change?.('MS Trollfjord' as never);
    buttonLabelled(ADD())?.click?.();

    expect(suggestionsFor(NAME())).toEqual(['Polar Aussenkabine', 'Arktis Superior']);
  });

  /** Both take anything typed: most airlines will never be a note, and neither will a riverboat somebody heard about once. */
  it('takes a ship and a carrier the vault has never heard of', () => {
    const { save } = openLeg();

    control(t('modals.tripEditor.legVehicle'), 'text')?.change?.('A nameless riverboat' as never);
    control(t('modals.tripEditor.legCarrier'), 'text')?.change?.('Helvetic Airs' as never);

...
export function aStop(over: Partial<TravelTripStop> = {}): TravelTripStop {
  return { ...aParsedStop(), target: null, targetKind: null, excursion: null, ...over };
}
...
/** An excursion as the board leaves one: a tour with its own words and no price anywhere. */
export function anExcursion(title: string, over: Partial<TravelExcursion> = {}): TravelExcursion {
  return {
    file: aFile(title),
    title,
    description: null,

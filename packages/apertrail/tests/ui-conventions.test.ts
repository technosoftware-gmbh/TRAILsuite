...
const MAY_QUERY: Record<string, string> = {
  'ui/gallery/travel-gallery-view.ts':
    'reads the outgoing search box before a full redraw discards it, to put focus and the caret back -- the grid filters on every keystroke, so without it the box loses focus after one character; the old subtree is thrown away on purpose, so there is no reference left to keep, and the read is null-guarded',
};

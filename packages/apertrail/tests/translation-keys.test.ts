...
const DYNAMIC_KEYS = [
  ...
  // The subfolder rows in the same catalogue, whose labels are the whole key
  // rather than a bare folder name.
  ...['tripBookingsSubfolder', 'exportsSubfolder'].flatMap((folder) => [
    `settings.folders.${folder}.name`,
    `settings.folders.${folder}.desc`,
    `settings.folders.${folder}.placeholder`,
  ]),
...
/**
 * A settings label is a settings label.
 *
 * `settings.properties.fields.<x>.name` reads "Rating property" -- right on
 * the page where you configure which frontmatter key to use, and wrong
 * everywhere else. The ship's brochure printed "Baujahr-Eigenschaft" in its
 * grey box from the day it shipped, the cabins dialog labelled its fields the
 * same way, and the prospect inherited both. `fieldNames.*` is the same field
 * as a plain noun.
 *
 * A source scan rather than a review habit, because the labels are correct in
 * the one place they belong and the mistake is only visible on the page.
 */
describe('settings labels stay on the settings page', () => {
  const SETTINGS_PAGES = join(SRC, 'ui', 'settings');

  it('names no settings property label outside the settings pages', () => {
    const offenders: string[] = [];
    for (const file of sourceFiles(SRC)) {
      if (file.startsWith(SETTINGS_PAGES)) continue;
      if (readFileSync(file, 'utf8').includes('settings.properties.fields.')) {

...
/**
 * A day the legs ask for.
 *
 * The outbound flight is often on a day the trip itself does not have: it
 * leaves the evening before day one, or lands the day after the last day.
 * Before this, a leg on such a day was named nowhere at all, which is how a
 * day one whose whole content was the flight out came to print the day and
 * say nothing about the flight.
 */
describe('a day drawn for a leg', () => {
  function leg(day: number | null, toDay: number | null = null) {
    return { day, toDay, from: null, to: null };
  }

  it('draws the day a leg leaves on when nothing else is on it', () => {
    const groups = itineraryDays([onDay('Oslo', 1)], null, [], [leg(0, 1)]);

    expect(groups.map((g) => g.number)).toEqual([0, 1]);
    expect(groups[0].stops).toEqual([]);
  });

  it('draws the day a leg lands on after the last day of the trip', () => {
    const groups = itineraryDays([onDay('Kopenhagen', 16)], null, [], [leg(16, 18)]);

    expect(groups.map((g) => g.number)).toEqual([16, 18]);
  });

  it('adds nothing to a day the stops already reach', () => {
    const groups = itineraryDays([onDay('Zürich', 1)], null, [], [leg(1, 1)]);

    expect(groups.map((g) => g.number)).toEqual([1]);
    expect(groups[0].stops).toHaveLength(1);
  });

  it('gives the drawn day its date once the trip has a departure', () => {
    const groups = itineraryDays([onDay('Oslo', 1)], '2027-12-16', [], [leg(0, 1)]);

    expect(groups[0].date).toBe('2027-12-15');
  });

  /**
   * `day: 1` with `toDay: 0` is what somebody writes when they read the
   * arrival field as "how many days later". It is not an arrival on day 0,
   * and drawing an empty day 0 for it would put a day before the trip that
   * nothing is ever named on.
   */
  it('draws no day for an arrival earlier than the departure', () => {
    const groups = itineraryDays([onDay('Oslo', 1)], null, [], [leg(1, 0)]);

    expect(groups.map((g) => g.number)).toEqual([1]);
  });

  it('draws nothing for a leg that says no day at all', () => {
    const groups = itineraryDays([onDay('Oslo', 1)], null, [], [leg(null, null)]);

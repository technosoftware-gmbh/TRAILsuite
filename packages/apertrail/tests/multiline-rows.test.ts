 ...
 * `apt-form-multiline`'s predecessor was on exactly one field of one dialog
...
const CLASS = 'apt-form-multiline';
...
describe('multiline setting rows', () => {
  ...
  it('has rows to check, so a broken scan cannot pass silently', () => {
    // A floor rather than a census. What it guards is the scan: a
    // `settingChains` that stopped matching would make the check below pass
    // over nothing, which is this package's own recurring defect in a test.
    const rows = files

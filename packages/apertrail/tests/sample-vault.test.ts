...
describe('the sample notes', () => {
  ...
  it('is nineteen notes across the folders the default settings name', () => {
    ...
    expect(sampleFolders(notes)).toEqual([
      ...
      'Places/Vehicles',
      // Nor is an excursion a place, for the same reason and in the same root.
      'Places/Excursions',
      // A trip is a folder of its own, named after the trip: see trip-folder.ts.
      'Trips/Rovos Rail 2026',
      // The one that follows another: it names Rovos Rail 2026 in its own
      // `extends:`, and the voyage note says nothing about it.
      'Trips/Winelands 2026',
...
describe('the seeded vault, read back', () => {
  it('parses as two countries, one state, three cities, five places and three trips', async () => {
    ...
    expect(board.trips.map((t) => t.title)).toEqual([
      'Aargau Weekend',
      'Rovos Rail 2026',
      'Winelands 2026',
    ]);
  ...
  it('reads a finished trip whole: stops, a rated stop, nights, legs, people and money', async () => {
    ...
    // Every stop that names a place resolves to a note in the set rather than
    // to a dangling link. The excursion line names none, on purpose: a tour
    // round a peninsula is not a visit to one note, and undefined here is the
    // absence of a link rather than a link that failed.
    ...
    expect(rovos?.stops.map((stop) => stop.targetKind)).toEqual([
      'city',
      null,
      'landmark',
      'photospot',
    ]);
    // In the note's own order, which is the itinerary's order: the peninsula
    // day is the 12th and Table Mountain the 13th, and a day written out of
    // sequence would open a second heading below the day after it.
    // And it resolves the other way instead: to the excursion note beside it.
    expect(rovos?.stops.map((stop) => stop.excursion?.title)).toEqual([
      undefined,
      'Cape Peninsula by road',
      undefined,
      undefined,
    ]);

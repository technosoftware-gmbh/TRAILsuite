...
/**
 * Which notes open with a related-trips fence.
 *
 * A Country and a State did not, and the reason recorded against it was that a
 * trip's stops point at cities and places, so the block would always be empty
 * there. The block has since learned to roll a region up -- a State asks the
 * question of its cities, a Country of its states' cities and of its own name
 * -- so the answer stopped being empty and nothing said so. Meanwhile the
 * prospect grew the same section for the same six notes, and the two surfaces
 * disagreed about a country.
 *
 * Asserted over every creator rather than over the two that changed, because
 * what this pins is which notes answer the question at all.
 */
describe('the related-trips block a note opens with', () => {
  const fence = '```travel-related-trips';

  it('is seeded into a country', async () => {
    const { app, created } = makeFakeVault();
    await createCountryNote(app, settings, 'Austria');
    expect(created[0].content).toContain(fence);
  });

  it('is seeded into a state', async () => {
    const { app, created } = makeFakeVault();
    await createStateNote(app, settings, 'Tyrol');
    expect(created[0].content).toContain(fence);
  });

  it('is still seeded into a city and a place', async () => {
    const city = makeFakeVault();
    await createCityNote(city.app, settings, 'Vienna');
    expect(city.created[0].content).toContain(fence);

    const landmark = makeFakeVault();
    await createLandmarkNote(landmark.app, settings, 'Stephansdom');
    expect(landmark.created[0].content).toContain(fence);
  });

  /** One fence, not two: a photo spot's own block sits above this one rather than replacing it. */
  it("sits under a photo spot's own block rather than instead of it", async () => {
    const { app, created } = makeFakeVault();
    await createPhotoSpotNote(app, settings, 'Rheinfall');
    const content = created[0].content;
    expect(content.indexOf('```apt-photo-spot')).toBeLessThan(content.indexOf(fence));

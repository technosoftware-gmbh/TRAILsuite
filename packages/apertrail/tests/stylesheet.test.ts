 ...
 *
 * Each is the `id` of a `<datalist>` an input points at through its `list`
 ...
 *
 * The scan reads quoted strings and cannot tell where one is used, so these are
 * named here rather than the scan made cleverer. A regex that tried to tell
 * a `cls:` from an `attr: { id }` would be a second parser to keep right.
 */
const ELEMENT_IDS = new Set([
  'apt-crm-tag-suggestions',
  'apt-travel-type-suggestions',
  // The leg editor's three cascading fields: who runs it, which ship, which
  // cabin. Each suggests what the vault has and takes anything typed, so each
  // needs a `<datalist>` of its own -- one shared id would make one field show
  // another's suggestions, which reads as a bug rather than as a shared list.
  'apt-leg-carrier-suggestions',
  'apt-leg-vehicle-suggestions',
  'apt-variant-name-suggestions',
  // The stop editor's excursion field, which suggests what the vault has and
  // takes anything typed, for the reason the leg's ship does.

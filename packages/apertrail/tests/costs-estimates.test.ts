...
/**
 * What a stop is called on a cost row.
 *
 * Reported off the real Nordkap plan: its optional rows read "Stavanger CHF
 * 238" and "Stavanger CHF 170", one above the other, and the excursion each
 * figure belongs to was the one thing missing. Two excursions in one port is
 * the ordinary case on a cruise, not the odd one.
 */
describe('a stop on a cost row', () => {
  it('names the excursion after the place', () => {
    expect(
      stopLabel(
        stop({ placeTitle: 'Stavanger', excursionTitle: 'Auf den Spuren der Wikinger' }),
        'Stop'
      )
    ).toBe('Stavanger · Auf den Spuren der Wikinger');
  });

  it('names the place alone when the stop is not an excursion', () => {
    expect(stopLabel(stop({ placeTitle: 'Tromsø', excursionTitle: null }), 'Stop')).toBe('Tromsø');
  });

  /** A stop that names an excursion and no place still says something worth reading. */
  it('names the excursion alone when there is no place', () => {
    expect(
      stopLabel(stop({ placeTitle: null, excursionTitle: 'Hundeschlittenfahrt' }), 'Stop')
    ).toBe('Hundeschlittenfahrt');
  });

  it('falls back to the given word when the stop names neither', () => {
    expect(stopLabel(stop({ placeTitle: null, excursionTitle: null }), 'Stop')).toBe('Stop');
  });

  /**
   * The label grew; the reference did not. A booking supersedes this line by
   * naming the place, and an excursion note is not a booking.
   */
  it('leaves the reference as the place', () => {
    const [row] = tripItemEstimates(
      trip({
        stops: [
          stop({
            placeTitle: 'Stavanger',
            excursionTitle: 'Auf den Spuren der Wikinger',
            cost: 119,
            costUnit: 'person',
          }),
        ],
    ...
    expect(row.label).toBe('Stavanger · Auf den Spuren der Wikinger');
    expect(row.reference).toBe('Stavanger');
  });
});

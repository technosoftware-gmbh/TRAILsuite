...
describe('a follow-up whose text says more than the fields do', () => {
  it('still writes a link somebody typed into the text', () => {
    // Typed into the box rather than chosen from the dropdown. It goes out
    // unchanged, and `childTasks()` reads it back into the project field on
    // the next open, because it is where a link belongs on the line.
    const lines = meeting({ followUps: [fu('Simulator testen [[PMQ Reconstitution]]')] });
    expect(parseTaskLine(lines[1])?.links).toEqual(['PMQ Reconstitution']);
  });
  ...
  it('leaves a date in the text alone even when the row has one too', () => {
    // The row's field loses, and it has to. A date still sitting in the text
    // is one somebody typed there, since `childTasks()` lifts a trailing one
    // into the row, and appending a second marker would leave one line saying
    // two different things -- after which it never round-trips again and its
    // meeting silently stops being editable.

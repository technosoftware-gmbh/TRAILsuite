...
describe('a simple posting', () => {
  it('is one line, and reads back as what went in', () => {
    const lines = linesFor({
      posting: posting({ debit: 4031, credit: 1011, amount: 750.95, text: 'Musterversicherung' }),
      legs: [],
    });
...
describe('a split', () => {
  const lines = linesFor({
    posting: posting({}),
    legs: [
      { account: 4031, amount: 750.95, text: 'Musterversicherung' },
  ...
  it('is a header and one indented line per leg', () => {
    ...
    expect(lines[1]).toBe('    4031 | 750.95 | Musterversicherung');
  });

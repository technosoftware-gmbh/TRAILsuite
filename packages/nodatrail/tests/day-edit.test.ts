...
import { copyDraft, emptyDraft, entryLines } from '../src/plan/add-to-day';
...
describe('what may be edited', () => {
  ...
  it('stays editable after a follow-up has been moved to another day', async () => {
    ...
    expect(meetings[0].draft.followUps).toEqual([
      { text: 'Nachfassen', context: '', due: '2026-09-07' },
    ]);
    expect(meetings[0].editable).toBe(true);
  });

  it('puts the project and the date in the fields that hold them', async () => {
    // Not in the text box. They used to be, and the two controls beside it
    // were then worse than empty: setting the date did nothing at all, and
    // choosing a project appended a second link after the date.
    const { meetings } = await entriesOf(
      `${SCHEDULE}\n\n- 👥 11:00-12:00 SW cases\n    - [ ] Check problem report [[CN-1099213]] 📅 2026-09-14\n`
    );
    expect(meetings[0].draft.followUps).toEqual([
      { text: 'Check problem report', context: 'CN-1099213', due: '2026-09-14' },
    ]);
  });

  it('writes the row date when the date box is changed', async () => {
    const { meetings } = await entriesOf(
      `${SCHEDULE}\n\n- 👥 11:00-12:00 SW cases\n    - [ ] Check problem report [[CN-1099213]] 📅 2026-09-14\n`
    );
    const draft = copyDraft(meetings[0].draft);
    draft.followUps[0].due = '2026-09-30';
    expect(entryLines(DEFAULT_SETTINGS, draft)[1]).toBe(
      '    - [ ] Check problem report [[CN-1099213]] 📅 2026-09-30'
    );
  });

  it('replaces the project rather than adding a second link', async () => {
    const { meetings } = await entriesOf(
      `${SCHEDULE}\n\n- 👥 11:00-12:00 SW cases\n    - [ ] Check problem report [[CN-1099213]] 📅 2026-09-14\n`
    );
    const draft = copyDraft(meetings[0].draft);
    draft.followUps[0].context = 'Beruf';
    const line = entryLines(DEFAULT_SETTINGS, draft)[1];
    expect(line).toBe('    - [ ] Check problem report [[Beruf]] 📅 2026-09-14');
    expect((line.match(/\[\[/g) ?? []).length).toBe(1);
  });

  it('keeps whatever the three fields cannot hold in the text', async () => {
    // A priority marker sits between the link and the date, so lifting the
    // link out would compose the line back in a different order. It stays put,
    // the row is coarser to edit, and the line survives.
    const { meetings } = await entriesOf(
      `${SCHEDULE}\n\n- 👥 11:00 PMQ\n    - [ ] Nachfassen [[PMQ]] ⏫ 📅 2026-09-14\n`
    );
    expect(meetings[0].draft.followUps).toEqual([
      { text: 'Nachfassen [[PMQ]] ⏫', context: '', due: '2026-09-14' },
    ]);
    expect(meetings[0].editable).toBe(true);
  });

  it('leaves a line holding two dates whole', async () => {
    // One date field, two dates. Lifting one would come back holding one of
    // them and the meeting would go read-only for a line the dialog was
    // perfectly able to leave alone.
    const { meetings } = await entriesOf(
      `${SCHEDULE}\n\n- 👥 11:00 PMQ\n    - [ ] Nachfassen ⏳ 2026-09-10 📅 2026-09-14\n`
    );
    expect(meetings[0].draft.followUps).toEqual([
      { text: 'Nachfassen ⏳ 2026-09-10 📅 2026-09-14', context: '', due: '' },
    ]);
    expect(meetings[0].editable).toBe(true);
  });

  it('composes every shape of follow-up back to the line it came from', async () => {
    // The arbiter, and the reason the reader above is allowed to be greedy:
    // whatever it lifts out has to go back in the same order, or the meeting
    // stops being editable and somebody loses the dialog rather than a line.
    const children = [
      'Nachfassen',
      'Nachfassen [[PMQ]]',
      'Nachfassen 📅 2026-09-14',
      'Nachfassen [[PMQ]] 📅 2026-09-14',
      'Nachfassen [[PMQ]] ⏫ 📅 2026-09-14',
      'Nachfassen [[A]] und [[B]] 📅 2026-09-14',
      'Nachfassen ⏳ 2026-09-10 📅 2026-09-14',
      'Nachfassen 🛫 2026-09-10 📅 2026-09-14',
      'Nachfassen #tag [[PMQ]] 📅 2026-09-14',
      '[[PMQ]]',
      // A day written the way nothing here writes one. Left alone rather than
      // tidied into the format this plugin prefers.
      'Nachfassen [[PMQ]] 📅 2026-9-4',
    ];

    for (const child of children) {
      const { meetings } = await entriesOf(`${SCHEDULE}\n\n- 👥 11:00 PMQ\n    - [ ] ${child}\n`);
      expect(meetings[0].editable).toBe(true);
      expect(entryLines(DEFAULT_SETTINGS, meetings[0].draft)[1]).toBe(`    - [ ] ${child}`);
    }
  });
...
describe('the draft the dialog hands to the editor', () => {
  it('keeps the record it was copied from out of reach', async () => {
    // The guard re-composes `entry.draft` and compares it with the note. So a
    // copy that shares `followUps` with the record means adding a follow-up
    // edits the very thing the guard measures against: the note can never
    // match again and the entry refuses to save for good. Reloading the view
    // does not help, because the next copy shares the same array.
    const { meetings } = await entriesOf(`${SCHEDULE}\n\n- 👥 11:00-12:00 SW cases\n`);
    const entry = meetings[0];
    const before = entryLines(DEFAULT_SETTINGS, entry.draft);

    const editing = copyDraft(entry.draft);
    editing.followUps.push({ text: 'Check if it is the known one', context: 'Beruf', due: '' });
    editing.text = 'SW cases, umbenannt';

    expect(entryLines(DEFAULT_SETTINGS, entry.draft)).toEqual(before);
  });

  it('keeps the rows out of reach too, not only the array', () => {
    // `listEditor` edits a row object in place rather than replacing it, which
    // is deliberate and documented where it happens. Copying the array alone
    // would therefore still write through to the record's own rows.
    const draft = {
      ...emptyDraft('meeting'),
      followUps: [{ text: 'Check the known one', context: 'Beruf', due: '' }],
    };
    const editing = copyDraft(draft);
    editing.followUps[0].text = 'etwas anderes';

    expect(draft.followUps[0].text).toBe('Check the known one');
  });
});
...
describe('the guard against a note that moved', () => {
  ...
  it('copies the draft it edits, rather than spreading it', () => {
    // The shallow spread this replaced shared `followUps` with the record, and
    // the guard below measures against exactly that. See `copyDraft`.
    expect(source).toContain('copyDraft(editing.entry.draft)');
    expect(source).not.toContain('{ ...editing.entry.draft }');
  });

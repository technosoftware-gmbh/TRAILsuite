...
export default class NODAtrailPlugin extends Plugin {
  ...
  /**
   * Redraws every open view.
   *
   * **After the metadata cache has caught up, not before.** A write resolves
   * when the file is on disk; the cache is re-parsed afterwards, and a list
   ...
   *
   * `whenIndexed` waits for one event and unsubscribes, so nothing here starts
   * redrawing on somebody else's edits. No path is passed because none of the
   * dialogs on this side reports which file it wrote: in the moment after a
   * save the next note the cache re-reads is that note, and the backstop
   * covers the rest.
   */
  refreshViews(): void {
    void whenIndexed(this.app).then(() => this.redrawViews());
  }

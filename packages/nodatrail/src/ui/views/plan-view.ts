...
import { EditTaskModal, editableTaskFields } from '../modals/edit-task-modal';
  ...
  protected async renderBody(): Promise<void> {
    ...
    const deps: PeriodSectionDeps = {
      ...
      editTask: (task) => {
        ...
        // Never null in practice: the row only offers the action when it is
        // not. Checked again rather than asserted, because the row and this
        // are two reads of the same line and the cheap answer to "what if they
        // disagree" is to do nothing.
        if (!fields) return;
        void new EditTaskModal(
          {
            app: this.deps.app,
            getSettings: this.deps.getSettings,
            onChanged: () => void this.render(),

...
export interface PeriodSectionDeps {
  ...
  /**
   * Editing what a task says, when it is due and how urgent it is.
   *
   * Optional like the two beside it, and offered per row rather than per view:
   * a line carrying more than the form holds is not editable at all, and the
   * row shows no button rather than one that would shrink it.
   */
  editTask?: (task: VaultTask) => void;
  /**
...
export function renderPeriodTasks(
  ...
  for (const task of inPeriod) {
    ...
    // Changing what the task says. Only for a line the form can put back
    // exactly -- see `taskFieldsOf()` -- because four boxes cannot hold a
    // recurrence rule, and an editor that dropped one would be worse than no
    // editor at all. A row without the button is still opened by clicking it.
    const editable = deps.editTask ? editableTaskFields(task) : null;
    if (deps.editTask && editable) {
      rowIconAction(line, 'pencil', t('plan.editTask'), () => deps.editTask?.(task));
    }

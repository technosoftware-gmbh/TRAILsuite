...
export function sectionCard(containerEl: HTMLElement, title?: string, intro?: string): HTMLElement {
  ...
  if (intro) {
    containerEl.createEl('p', { cls: 'nod-settings-intro', text: intro });
  }
  return containerEl.createDiv({ cls: 'nod-settings-card' });
}
...
export function noteLine(containerEl: HTMLElement, text: string): void {
  containerEl.createEl('p', { cls: 'nod-settings-note', text });
}
...
export function navRow(containerEl: HTMLElement, options: NavRowOptions): void {
  ...
  setting.settingEl.addClass('nod-settings-nav-row');
  setting.settingEl.setAttr('role', 'button');
  setting.settingEl.setAttr('tabindex', '0');

  if (options.value) {
    setting.controlEl.createSpan({ cls: 'nod-settings-nav-value', text: options.value });
  }
  setIcon(setting.controlEl.createSpan({ cls: 'nod-settings-nav-chevron' }), 'chevron-right');
...
export function linkRow(
  ...
  for (const link of links) {
    const anchor = setting.controlEl.createEl('a', {
      cls: 'nod-settings-link-button',
      href: link.href,
      attr: { target: '_blank', rel: 'noopener' },
    });
    anchor.toggleClass('mod-cta', link.cta === true);
    if (link.icon) setIcon(anchor.createSpan({ cls: 'nod-settings-link-icon' }), link.icon);

...
/**
 * Rewrites one task's line from the fields a form holds.
 *
 * **The one writer here that rebuilds a line rather than patching it**, and it
 * is allowed to only because `taskFieldsOf()` refused every line it could not
 * put back character for character. A caller that skipped that check would be
 * offering to edit a recurrence rule it is about to drop.
 *
 * Returns false on the same terms as `setTask`: the note has moved on under
 * the view that scanned it, or the line already said this.
 */
export async function editTaskLine(
  ...
  return write(app, settings, task, replacing(task, composeTaskFields(task, fields)));
}

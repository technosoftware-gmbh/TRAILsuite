/**
 * One line under a day note's schedule heading, parsed. Pure: no note, no host.
 *
 * Split out of `read-schedule.ts` so the interchange export can read a day
 * note outside Obsidian; `read-schedule.ts` re-exports all of it, so nothing
 * that imported it from there had to change. What the rules are and why is
 * written beside each of them below.
 */
 ...
 */
export function sectionLines(body: string, headings: readonly string[]): string[] {

...
A stop can also say what it **is**, as against where it happens:
...
`excursion:` points at a note of its own, in `Places/Excursions`, holding the
tour's description, its picture, how long it takes and who runs it. **The price
stays on the trip**, because the same tour costs one thing at Christmas and
another in May, and the description stays on the excursion, so correcting it
corrects every trip that ever took it.

You do not need a note for it. A name you typed still reads on the page; only
the description and the picture need one. And a stop may name an excursion and
no place at all, for an outing that meets on board.

#### A trip that follows another trip

Three days in Kopenhagen after the ship docks is a trip of its own, with its own
dates, its own hotel and its own budget. Say so on **the second one**:

```yaml
---
type: trip
departure: 2027-12-31
extends: "[[Die Nordkap-Linie ab Oslo]]"
---
```

The voyage note is never edited: what follows a trip is worked out when the
vault is read. Its trip document gains a closing "Nachprogramm" section naming
the extension, with the extension's own total **beside** its own and never added
into it, because the two are separately booked and separately cancelled.

One thing to check by hand when you add one: a flight home that was on the first
trip usually belongs to the second one now. Nothing moves it for you, and
nothing should.
...
Place notes take `address`, `website`, `rating`, `tags` and `geoLocation` when
you want them, and you can fill them in from the note's own card: open the
3-dot menu on any accommodation, restaurant, landmark, location or photo spot
and choose **Edit**. Accommodation adds its kind and its booking status there,
and a restaurant its kind.

Every card's menu reads the same way: **Edit**, then the page that note prints
-- **Brochure** for a place, a city, a state, a country, an excursion or a
ship, **Trip document** for a trip. The same two entries sit in the note's own
related-trips block, so it makes no difference whether you reach a note from
the gallery or have it open in front of you.

A country or a state written before the block existed will not show its buttons
until it has one. Saving it from its editor once adds it, and a note that
already has one is not touched.

Every one of those dialogs ends with the same five rows, in the same order, as
a trip's: **Subtitle**, **Picture**, **Highlights**, **Summary**, **Gallery**.
Three of them used to be a second button called Titelbild, and it is one dialog
now, because a country with two buttons where the smaller one held a single
field was a strange way to ask somebody what a country is.

**Subtitle** is the one line, in the words a brochure would use. It is what
goes under the title on the printed page. **Highlights** is one line per line,
in the order they should read; the prospect prints them as a starred list under
the picture, which is where the trip document has always printed a trip's.

**Summary** is the long version. It is the summary callout in the note's body,
and it is what the prospect prints as prose: the whole callout, lists and links
included, so what you type in the box is what comes out on the page. Anything
you write in the note **below** the callout is your own working notes and stays
off the sheet, which is what makes the box worth having rather than a form with
a field per heading.

Three things worth knowing about that box. It leaves the callout's opening line
exactly as your note spells it, so a `> [!summary]` written by hand stays that
way and a collapsed one stays collapsed. A note with no callout gets one, above
whatever it already holds. And if a note somehow carries two summary callouts,
the first is the one you are editing and the dialog says so, because everything
that reads a summary reads the first.

Two things the editor deliberately leaves alone. It does not rename the note:
retitling a place is an ordinary Obsidian rename, which fixes every link
pointing at it, and a title box here could not do that. And it never writes
`visited` or `lastVisit`, because a finished trip that stops somewhere already
answers those, and two answers to one question disagree sooner or later.

An excursion and a ship are edited the same way, from the card or from the
note's own block. The ship's dialog is the one that used to be called Cabins:
it holds the facts and the cabin categories together, on purpose, because
those are what somebody fills in from an operator's page in one sitting.

A country, a state and a city are edited the same way too, from the card or
from the note's own block: a city carries its country, its state, its coordinates
...
or New country dialog, where you type the name and press Create; nothing is
written until you do. The new note is selected when you come back, and it stays
even if you then cancel the place you were filling in, so if you change your
mind, delete it the way you would delete any note.

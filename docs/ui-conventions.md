...
**A textarea in a setting row needs the row marked, not just the box.**
Obsidian lays a row out as info on the left and control on the right, and the
control takes only the width it asks for -- so a textarea at `width: 100%` is
100% of a column that does not grow, and stays a small square however tall it
is told to be. A description of any length squeezes it further. The row gets a
class that stretches the control and holds the description to a column beside
it: `nod-form-multiline`, `apt-form-multiline`.

This one has now been paid for three times. NODAtrail worked it out and wrote
it into `FormModal.multiline`; its own `summaryField`, one file away, added the
box class and not the row class, so the summary on all four PARA forms was the
square the rule exists to prevent. APERtrail had a narrower version of the fix
on exactly one field of one dialog, and a vehicle's cabin descriptions were
still four words wide when somebody filled in a real ship. **A helper that does
it right is not the rule; the rule is that every multiline row carries it**, and
a field built by hand beside the helper is where it goes missing.

**A field that names a file offers a way to pick one.** Typing a vault path
from memory means leaving the dialog to go and find it, which is the moment the
form gets abandoned. Every such field is a text box that stays editable -- a
value may be a URL, or a path to something the picker cannot list -- plus a
button that picks from the vault, plus, where the form knows which note it is
editing, a button that takes one from the machine and files it beside that note.
Where the form does not know the note, the upload button is not drawn: a form
that cannot say which note it is editing cannot say where a file of it should
go, and the vault root is not an answer.

**Pictures and documents are two fields, not one widened field.** They differ
in what they offer and in nothing else, and a picture picker listing PDFs is a
control that lies about what belongs in the field it fills. What counts as a
document is the wider net -- everything that is not a note -- because a deck
plan is a PDF far more often than not and a photograph of a printed one is the
same thing to whoever is filing it. That overlap is deliberate and is the only
place the two nets cross.
...
- **One note, one editor.** A note's fields are edited in one dialog, not
  split across two by which schema happens to own which key. The line under a
  note's title, its picture and its gallery were a dialog of their own while
  nothing else could edit those notes; the moment each of them had an editor,
  that left a country with two buttons of which the smaller held one field and
  the larger was not named after the note. They are a section inside each
  editor now, shared as a component rather than as a surface.
- **A menu offers Edit, then the page this note prints.** Wherever an entity's
  actions appear -- a gallery card's 3-dot menu, the related-trips block's
  button row -- they read in that order, so two surfaces showing the same
  entity cannot come to disagree about it. An entity with no action at all gets
  no menu rather than an empty one.
- **Editing says only "Edit", whatever the note is.** A menu is opened on one
  card or inside one note, so the surface has already said which note it means;
  naming the type a second time is one string per type to keep in step. The
  printed pages do keep their own words, because a prospect and a trip document
  are two different documents rather than one act named twice.
- **Both surfaces read their labels out of one map.** `CARD_ACTION_LABELS` and
  `BLOCK_ACTION_LABELS` name the same translation keys for the same acts, and a
  test says so: the defect this prevents is real, and was a landmark's card
  offering "Edit trip".
- **One dialog is one write.** An editor that collects two schemas' fields
  saves them in one pass over the note: two passes are two vault writes and two
  cache invalidations for one logical edit, and the second can fail on its own.
- **A dropdown that names another note still offers the value the note
  carries**, even when the vault no longer has that note. A place whose city
  was renamed still SAYS that city, and a control that quietly dropped the
  value erased the link the next time anything else on the note was saved.
- **Offering to create the missing note is an option in the list, not a side
  effect.** It opens that note's own creation dialog, which is where a name is
  typed and confirmed, and nothing is written until somebody presses Create
  there. A dialog opened from inside another form does not take the workspace
  with it: somebody creating a country while filling in a place is not asking
  to leave the place.

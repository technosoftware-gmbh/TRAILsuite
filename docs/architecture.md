...
| `vault-smoke` (both plugins) | A reader that works against invented frontmatter and not against a real vault |
...
| `interchange-copies` (suite) | The two plugins' copies of the interchange export's disk host, argument handling and launcher (`scripts/interchange/`) differing anywhere but the line that names the other plugin |
...
10. *(Closed 3 September 2026.)* **Three vault-reading suites, three mechanisms,
    ...
    `NODATRAIL_VAULT`, or `APERTRAIL_VAULT` for the other one, which is what it
    is for -- it checks the shapes a long-lived vault actually holds, which is
    a different question from the one the seeders answer.
    ...
    **Reading `data.json` is not the whole of it, which APERtrail's copy found
    on 7 September 2026.** A folder key the vault never saved is not in that
    file at all, and the fallback runs through the localized defaults, which
    need an initialized `I18nManager` and in a test process resolve to English.
    Two German folders were therefore looked for under English names. So that
    suite's first real assertion does not loop over what it found: it scans the
    whole vault for each `type:` value and reports any note the reader missed,
    naming the folder it looked in. `APERTRAIL_LOCALE=de` settles the fallback
    when a vault has never saved its folders.

    **And a harness that reads the filesystem has to normalize the way Obsidian
    normalizes.** macOS and iCloud store filenames decomposed, so
    `Gifthüttli.md` on disk is `u` plus a combining diaeresis while the
    wikilink typed into a note is precomposed; they compare unequal and every
    non-ASCII title looks like a broken link. Obsidian normalizes to NFC before
    anything sees a path, so the plugin never meets this and the first run of
    the new suite reported a perfectly good link as broken.
